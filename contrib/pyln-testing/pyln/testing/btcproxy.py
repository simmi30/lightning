""" A brocoind proxy that allows instrumentation and canned responses
"""
from flask import Flask, request  # type: ignore
from brocoin.rpc import JSONRPCError  # type: ignore
...
class BrocoinRpcProxy(object):
    def __init__(self, brocoind, rpcport=0):
        self.app = Flask("BrocoindProxy")
    ...
    def _handle_request(self, r):
        brpc = BrocoinProxy(bron_conf_file=self.brocoind.conf_file)
        method = r['method']
    ...
    def start(self):
        ...
        # Now that brocoind is running on the real rpcport, let's tell all
        ...
        logging.debug("BrocoinRpcProxy proxying incoming port {} to {}".format(self.rpcport, self.brocoind.rpcport))

    def stop(self):
        self.server.stop()
        self.proxy_thread.join()
        logging.debug("BrocoinRpcProxy shut down after processing {} requests".format(self.request_count))
    ...
    def mock_rpc(self, method, response=None):
        ...
        is removed and future calls will be passed through to brocoind again.
