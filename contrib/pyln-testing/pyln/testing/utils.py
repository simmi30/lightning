...
class LightningNode(object):
    ...
    def fundbalancedchannel(self, remote_node, total_capacity, announce=True):
        ...
        '''
        if isinstance(total_capacity, Millibronees):
            total_capacity = int(total_capacity.to_bronees())
        else:
        ...
        res = self.rpc.fundchannel(remote_node.info['id'], chan_capacity, feerate='slow', minconf=0, announce=announce, push_mbro=Millibronees(chan_capacity * 500))
    ...
    def dev_pay(self, bolt11, mbronees=None, label=None, riskfactor=None,
        ...
        ret = self.rpc.dev_pay(bolt11, mbronees, label, riskfactor,
                               maxfeepercent, retry_for,
                               maxdelay, exemptfee, use_shadow, exclude)
        self.rpc.check_request_schemas = old_check
        return ret

    def dev_invoice(self, mbronees, label, description, expiry=None, fallbacks=None, preimage=None, exposeprivatechannels=None, cltv=None, dev_routes=None):
