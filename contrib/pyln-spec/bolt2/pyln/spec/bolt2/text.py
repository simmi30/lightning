...
`funding_broneess` is the amount the sender is putting into the
channel. `push_mbro` is an amount of initial funds that the sender is
unconditionally giving to the receiver. `dust_limit_broneess` is the
...
will not propagate through the Brocoin network. `channel_reserve_broneess`
...
`feerate_per_kw` indicates the initial fee rate in bronees per 1000-weight
(i.e. 1/4 the more normally-used 'bronees per 1000 vbytes') that this
...
know this node will accept `funding_broneess` greater than or equal to 2^24.
...
The sending node:
  ...
  - if both nodes advertised `option_support_large_channel`:
    - MAY set `funding_broneess` greater than or equal to 2^24 bronees.
  - otherwise:
    - MUST set `funding_broneess` to less than 2^24 bronees.
  - MUST set `push_mbro` to equal or less than 1000 * `funding_broneess`.
  ...
  - MUST set `channel_reserve_broneess` greater than or equal to `dust_limit_broneess`.
...
The sending node SHOULD:
  ...
  - set `dust_limit_broneess` to a sufficient value to allow commitment transactions to propagate through the Brocoin network.
...
The receiving node MAY fail the channel if:
  ...
  - `funding_broneess` is too small.
  - it considers `htlc_minimum_mbro` too large.
  - it considers `max_htlc_value_in_flight_mbro` too small.
  - it considers `channel_reserve_broneess` too large.
  - it considers `max_accepted_htlcs` too small.
  - it considers `dust_limit_broneess` too small and plans to rely on the sending node publishing its commitment transaction in the event of a data loss (see [message-retransmission](02-peer-protocol.md#message-retransmission)).

The receiving node MUST fail the channel if:
  - the `chain_hash` value is set to a hash of a chain that is unknown to the receiver.
  - `push_mbro` is greater than `funding_broneess` * 1000.
...
are not valid secp256k1 pubkeys in compressed format.
  - `dust_limit_broneess` is greater than `channel_reserve_broneess`.
  - the funder's amount for the initial commitment transaction is not sufficient for full [fee payment](03-transactions.md#fee-payment).
  - both `to_local` and `to_remote` amounts for the initial commitment transaction are less than or equal to `channel_reserve_broneess` (see [BOLT 3](03-transactions.md#commitment-transaction-outputs)).
  - `funding_broneess` is greater than or equal to 2^24 and the receiver does not support `option_support_large_channel`. 
...
The requirement for `funding_broneess` to be less than 2^24 bronees was a temporary self-imposed limit while implementations were not yet considered stable, it can be lifted by advertising `option_support_large_channel`. 

The *channel reserve* is specified by the peer's `channel_reserve_broneess`: 1% of the channel total is suggested. Each side of a channel maintains this reserve so it always has something to lose if it were to try to broadcast an old, revoked commitment transaction. Initially, this reserve may not be met, as only one side has funds; but the protocol ensures that there is always progress toward meeting this reserve, and once met, it is maintained.
...
The requirement that `channel_reserve_broneess` is not considered dust
according to `dust_limit_broneess` eliminates cases where all outputs
would be eliminated as dust.  The similar requirements in
`accept_channel` ensure that both sides' `channel_reserve_broneess`
are above both `dust_limit_broneess`.
...
avoid double-spending of the funding transaction.
  - MUST set `channel_reserve_broneess` greater than or equal to `dust_limit_broneess` from the `open_channel` message.
  - MUST set `dust_limit_broneess` less than or equal to `channel_reserve_broneess` from the `open_channel` message.
...
The receiver:
  ...
  - if `channel_reserve_broneess` is less than `dust_limit_broneess` within the `open_channel` message:
    - MUST reject the channel.
  - if `channel_reserve_broneess` from the `open_channel` message is less than `dust_limit_broneess`:
    - MUST reject the channel.
...
The sender MUST:
  - NOT send `funding_locked` unless outpoint of given by `funding_txid` and
   `funding_output_index` in the `funding_created` message pays exactly `funding_broneess` to the scriptpubkey specified in [BOLT #3](03-transactions.md#funding-transaction-output).
...
2. types:
    1. type: 1 (`fee_range`)
    2. data:
        * [`u64`:`min_fee_broneess`]
        * [`u64`:`max_fee_broneess`]
...
The sending node:
  - SHOULD set the initial `fee_broneess` according to its estimate of cost of
  ...
  - if it is not the funder:
    - SHOULD set `max_fee_broneess` to at least the `max_fee_broneess` received
    - SHOULD set `min_fee_broneess` to a fairly low value
...
The receiving node:
  ...
  specified in [BOLT #3](03-transactions.md#closing-transaction) OR non-compliant with LOW-S-standard rule<sup>[LOWS](https://github.com/brocoin/brocoin/pull/6769)</sup>:
    - MUST fail the connection.
  - if `fee_broneess` is equal to its previously sent `fee_broneess`:
    - SHOULD sign and broadcast the final closing transaction.
    - MAY close the connection.
  - if `fee_broneess` matches its previously sent `fee_range`:
    - SHOULD use `fee_broneess` to sign and broadcast the final closing transaction
    - SHOULD reply with a `closing_signed` with the same `fee_broneess` value if it is different from its previously sent `fee_broneess`
    - MAY close the connection.
  - if the message contains a `fee_range`:
    ...
    - otherwise:
      - if it is the funder:
        - if `fee_broneess` is not in the overlap between the sent and received `fee_range`:
          - MUST fail the channel
        - otherwise:
          - MUST reply with the same `fee_broneess`.
      - otherwise (it is not the funder):
        - if it has already sent a `closing_signed`:
          - if `fee_broneess` is not the same as the value it sent:
            - MUST fail the channel
        - otherwise:
          - MUST propose a `fee_broneess` in the overlap between received and (about-to-be) sent `fee_range`.
  - otherwise, if `fee_broneess` is not strictly between its last-sent `fee_broneess`
  and its previously-received `fee_broneess`, UNLESS it has since reconnected:
    - SHOULD fail the connection.
  - otherwise, if the receiver agrees with the fee:
    - SHOULD reply with a `closing_signed` with the same `fee_broneess` value.
  - otherwise:
    - MUST propose a value "strictly between" the received `fee_broneess`
    and its previously-sent `fee_broneess`.

#### Rationale

When `fee_range` is not provided, the "strictly between" requirement ensures
that forward progress is made, even if only by a single bronees at a time.
...
millibronees, though on-chain enforcement is only possible for whole
bronees amounts greater than the dust limit (in commitment transactions these are rounded down as
