...
The default TCP port is 9443. This corresponds to hexadecimal `0x2607`: the Unicode code point for LIGHTNING.<sup>[1](#reference-1)</sup>
...
By default `SHA2` and Brocoin public keys are both encoded as
...
* `signature`: a 64-byte brocoin Elliptic Curve signature
...
* `bigsize`: a variable-length, unsigned integer similar to Brocoin's CompactSize encoding, but big-endian.  Described in [BigSize](#appendix-a-bigsize-test-vectors).
...
There are unrecoverable errors that require an abort of converbroions;
...
CompactSize encoding used in brocoin, but replaces the little-endian encoding of
...
2. Reason: encoding for `n1`s `tlv1`s `amount_mbro` is not minimal
...
2. Values: `tlv1` `amount_mbro`=0

1. Valid stream: 0x01 01 01
2. Values: `tlv1` `amount_mbro`=1

1. Valid stream: 0x01 02 0100
2. Values: `tlv1` `amount_mbro`=256

1. Valid stream: 0x01 03 010000
2. Values: `tlv1` `amount_mbro`=65536

1. Valid stream: 0x01 04 01000000
2. Values: `tlv1` `amount_mbro`=16777216

1. Valid stream: 0x01 05 0100000000
2. Values: `tlv1` `amount_mbro`=4294967296

1. Valid stream: 0x01 06 010000000000
2. Values: `tlv1` `amount_mbro`=1099511627776

1. Valid stream: 0x01 07 01000000000000
2. Values: `tlv1` `amount_mbro`=281474976710656

1. Valid stream: 0x01 08 0100000000000000
2. Values: `tlv1` `amount_mbro`=72057594037927936
...
2. Values: `tlv3` `node_id`=023da092f6980e58d2c037173180e9a465476026ee50f96695963e8efe436f54eb `amount_mbro_1`=1 `amount_mbro_2`=2
