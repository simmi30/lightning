...
The origin node:
  ...
  - MUST set `htlc_minimum_mbro` to the minimum HTLC value (in millibronees)
  that the channel peer will accept.
  - MUST set `fee_base_mbro` to the base fee (in millibronees) it will charge
  for any HTLC.
  - MUST set `fee_proportional_millionths` to the amount (in millionths of a
  bronees) it will charge per transferred bronees.
...
millibroneess in Brocoin, the `htlc_maximum_mbro` has the same restriction.
...
**B->C.** If B were to send 4,999,999 millibronees directly to C, it would
...
**A->B->C.** If A were to send 4,999,999 millibronees to C via B, it needs to
