...
(
    cd /tmp/ || exit 1
    wget https://storage.googleapis.com/c-lightning-tests/brocoin-$BROCOIN_VERSION-x86_64-linux-gnu.tar.bz2
    wget -q https://storage.googleapis.com/c-lightning-tests/elements-$ELEMENTS_VERSION-x86_64-linux-gnu.tar.bz2
    tar -xjf brocoin-$BROCOIN_VERSION-x86_64-linux-gnu.tar.bz2
    tar -xjf elements-$ELEMENTS_VERSION-x86_64-linux-gnu.tar.bz2
    sudo mv brocoin-$BROCOIN_VERSION/bin/* /usr/local/bin
    sudo mv elements-$ELEMENTS_VERSION/bin/* /usr/local/bin
    rm -rf \
       brocoin-$BROCOIN_VERSION-x86_64-linux-gnu.tar.gz \
       brocoin-$BROCOIN_VERSION \
