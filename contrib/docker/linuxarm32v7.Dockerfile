...
# * downloader: Download litecoin/brocoin and qemu binaries needed for c-lightning
...
ARG BROCOIN_VERSION=0.18.1
ENV BROCOIN_TARBALL brocoin-$BROCOIN_VERSION-arm-linux-gnueabihf.tar.gz
...
RUN mkdir /opt/brocoin && cd /opt/brocoin \
    ...
    && grep $BROCOIN_TARBALL brocoin.asc | tee SHA256SUMS.asc \
