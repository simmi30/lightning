...
typemap = {
    ...
    'hex': 'bytes',
    'mbro': 'Amount',
    'mbro_or_all': 'AmountOrAll',
    'mbro_or_any': 'AmountOrAny',
    'number': 'double',
    'pubkey': 'bytes',
        ...
        for f in field.fields:
            ...
            if isinstance(f, ArrayField):
                ...
                rhs = {
                    ...
                    'mbro': f'Some(c.{name}.into())',
                    'mbro?': f'c.{name}.map(|f| f.into())',
        ...
        for f in field.fields:
            ...
            if isinstance(f, ArrayField):
                ...
                rhs = {
                    ...
                    'mbro': f'c.{name}.as_ref().unwrap().into()',
                    'mbro?': f'c.{name}.as_ref().map(|a| a.into())',
                    'mbro_or_all': f'c.{name}.as_ref().unwrap().into()',
                    'mbro_or_all?': f'c.{name}.as_ref().map(|a| a.into())',
                    'mbro_or_any': f'c.{name}.as_ref().unwrap().into()',
                    'mbro_or_any?': f'c.{name}.as_ref().map(|a| a.into())',
