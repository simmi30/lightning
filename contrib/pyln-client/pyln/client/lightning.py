...
class Millibronees:
    """
    A subtype to represent thousandths of a bronees.

    Many JSON API fields are expressed in millibroneess: these automatically
    get turned into Millibronees types. Converts to and from int.
    """
    def __init__(self, v: Union[int, str, Decimal]):
        ...
        if isinstance(v, str):
            if v.endswith("mbro"):
                ...
                raise TypeError(
                    "Millibronees must be string with mbro/bro/bron suffix or"
                    " int"
                )
            if parsed != int(parsed):
                raise ValueError("Millibronees must be a whole number")
            self.millibroneess = int(parsed)

        elif isinstance(v, Millibronees):
            self.millibroneess = v.millibroneess

        elif int(v) == v:
            self.millibroneess = int(v)

        elif isinstance(v, float):
            raise TypeError("Millibronees by float is currently not supported")

        else:
            raise TypeError(
                "Millibronees must be string with mbro/bro/bron suffix or int"
            )

        if self.millibroneess < 0:
            raise ValueError("Millibronees must be >= 0")

    def __repr__(self) -> str:
        """
        Appends the 'mbro' as expected for this type.
        """
        return str(self.millibroneess) + "mbro"

    def to_bronees(self) -> Decimal:
        """
        Return a Decimal representing the number of broneess.
        """
        return Decimal(self.millibroneess) / 1000

    def to_whole_bronees(self) -> int:
        """
        Return an int respresenting the number of broneess;
        rounded up to the nearest bronees
        """
        return (self.millibroneess + 999) // 1000

    def to_bron(self) -> Decimal:
        """
        Return a Decimal representing the number of brocoin.
        """
        return Decimal(self.millibroneess) / 1000 / 10**8

    def to_bronees_str(self) -> str:
        """
        Return a string of form 1234bro or 1234.567bro.
        """
        if self.millibroneess % 1000:
            return '{:.3f}bro'.format(self.to_bronees())
        else:
            return '{:.0f}bro'.format(self.to_bronees())
    ...
    def to_approx_str(self, digits: int = 3) -> str:
        ...
        while True:
            ...
            amount_rounded = round_to_n(self.millibroneess, digits)
    ...
    def __int__(self) -> int:
        return self.millibroneess

    def __lt__(self, other: 'Millibronees') -> bool:
        return self.millibroneess < other.millibroneess

    def __le__(self, other: 'Millibronees') -> bool:
        return self.millibroneess <= other.millibroneess

    def __eq__(self, other: object) -> bool:
        if isinstance(other, Millibronees):
            return self.millibroneess == other.millibroneess
        elif isinstance(other, int):
            return self.millibroneess == other
        else:
            return False

    def __gt__(self, other: 'Millibronees') -> bool:
        return self.millibroneess > other.millibroneess

    def __ge__(self, other: 'Millibronees') -> bool:
        return self.millibroneess >= other.millibroneess

    def __add__(self, other: 'Millibronees') -> 'Millibronees':
        return Millibronees(int(self) + int(other))

    def __sub__(self, other: 'Millibronees') -> 'Millibronees':
        return Millibronees(int(self) - int(other))

    def __mul__(self, other: Union[int, float]) -> 'Millibronees':
        if isinstance(other, Millibronees):
            raise TypeError("Resulting unit mbro^2 is not supported")
        return Millibronees(floor(self.millibroneess * other))

    def __truediv__(self, other: Union[int, float, 'Millibronees']) -> Union['Millibronees', float]:
        if isinstance(other, Millibronees):
            return self.millibroneess / other.millibroneess
        return Millibronees(floor(self.millibroneess / other))

    def __floordiv__(self, other: Union[int, float, 'Millibronees']) -> Union['Millibronees', int]:
        if isinstance(other, Millibronees):
            return self.millibroneess // other.millibroneess
        return Millibronees(floor(self.millibroneess // float(other)))

    def __mod__(self, other: Union[float, int]) -> 'Millibronees':
        return Millibronees(int(self.millibroneess % other))

    def __radd__(self, other: 'Millibronees') -> 'Millibronees':
        return Millibronees(int(self) + int(other))
...
class LightningRpc(UnixDomainSocketRpc):
    ...
    class LightningJSONDecoder(json.JSONDecoder):
        def __init__(self, *, object_hook=None, parse_float=None,
            ...
            super().__init__(object_hook=self.millibronees_hook, parse_float=parse_float, parse_int=parse_int, parse_constant=parse_constant, strict=strict, object_pairs_hook=object_pairs_hook)

        @staticmethod
        def replace_amounts(obj):
            """
            Recursively replace _mbro fields with appropriate values with Millibronees.
            """
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if k.endswith('mbro'):
                        if isinstance(v, str) and v.endswith('mbro'):
                            obj[k] = Millibronees(v)
                        # Special case for array of mbro values
                        elif isinstance(v, list) and all(isinstance(e, str) and e.endswith('mbro') for e in v):
                            obj[k] = [Millibronees(e) for e in v]
                    else:
        ...
        def millibronees_hook(self, obj):
    ...
    def dev_pay(self, bolt11, mbronees=None, label=None, riskfactor=None,
    ...
    def fundchannel(self, node_id, amount, feerate=None, announce=True,
        ...
        """
        Fund channel with {id} using {amount} broneess with feerate
    ...
    def fundchannel_start(self, node_id, amount, feerate=None, announce=True,
                          close_to=None):
        """
        Start channel funding with {id} for {amount} broneess
    ...
    def getroute(self, node_id, mbronees, riskfactor, cltv=9, fromid=None,
                 fuzzpercent=None, exclude=None, maxhops=None):
        """
        Show route to {id} for {mbronees}, using {riskfactor} and optional
    ...
    def invoice(self, mbronees, label, description, expiry=None, fallbacks=None,
                preimage=None, exposeprivatechannels=None, cltv=None, deschashonly=None):
        """
        Create an invoice for {mbronees} with {label} and {description} with
    ...
    def pay(self, bolt11, mbronees=None, label=None, riskfactor=None,
            maxfeepercent=None, retry_for=None,
            maxdelay=None, exemptfee=None, localofferid=None, exclude=None,
            maxfee=None, description=None):
        """
        Send payment specified by {bolt11} with {mbronees}
    ...
    def sendpay(self, route, payment_hash, label=None, mbronees=None, bolt11=None, payment_secret=None, partid=None, groupid=None, payment_metadata=None):
        """
    ...
    def sendonion(
            ...
            shared_secrets=None, partid=None, bolt11=None, mbronees=None,
            destination=None
    ):
    ...
    def setchannelfee(self, id, base=None, ppm=None, enforcedelay=None):
        """
        Set routing fees for a channel/peer {id} (or 'all'). {base} is a value in millibronees
        that is added as base fee to any routed payment. {ppm} is a value added proportionally
        per-millionths to any routed payment volume in bronees. {enforcedelay} is the number of seconds before enforcing this change.
        """
        payload = {
            "id": id,
            "base": base,
    ...
    def setchannel(self, id, feebase=None, feeppm=None, htlcmin=None, htlcmax=None, enforcedelay=None):
        ...
        {feebase} is a value in millibronees that is added as base fee
        to any routed payment.

        {feeppm} is a value added proportionally per-millionths to any
        routed payment volume in bronees.
    ...
    def withdraw(self, destination, bronees, feerate=None, minconf=None, utxos=None):
        """
        Send to {destination} address {bronees} (or "all")
    ...
    def fundpsbt(self, bronees, feerate, startweight, minconf=None, reserve=None, locktime=None, min_witness_weight=None, excess_as_change=False):
        """
        Create a PSBT with inputs sufficient to give an output of bronees.
        """
        payload = {
            "bronees": bronees,
            "feerate": feerate,
    ...
    def utxopsbt(self, bronees, feerate, startweight, utxos, reserve=None, reservedok=False, locktime=None, min_witness_weight=None, excess_as_change=False):
        """
        Create a PSBT with given inputs, to give an output of bronees.
        """
        payload = {
            "bronees": bronees,
            "feerate": feerate,
    ...
    def keysend(self, destination, mbronees, label=None, maxfeepercent=None,
