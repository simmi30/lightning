from .lightning import LightningRpc, Millibronees
...
class Plugin(object):
    ...
    def _coerce_arguments(
        ...
        for key, val in ba.arguments.items():
            ...
            if annotation is not None and annotation == Millibronees:
                args[key] = Millibronees(val)
            else:
                args[key] = val
        ba.arguments = args
        return ba
    ...
    def _multi_dispatch(self, msgs: List[bytes]) -> bytes:
        ...
        for payload in msgs[:-1]:
            # Note that we use function annotations to do Millibronees
