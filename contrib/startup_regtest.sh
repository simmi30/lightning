...
## brocoind, all running on regtest
...
if [ -z "$PATH_TO_BROCOIN" ]; then
	if [ -d "$HOME/.brocoin" ]; then
		PATH_TO_BROCOIN="$HOME/.brocoin"
	elif [ -d "$HOME/Library/Application Support/Brocoin/" ]; then
		PATH_TO_BROCOIN="$HOME/Library/Application Support/Brocoin/"
	else
		echo "\$PATH_TO_BROCOIN not set to a .brocoin dir?" >&2
		return
	fi
fi
...
start_ln() {
	# Start brocoind in the background
	test -f "$PATH_TO_BROCOIN/regtest/brocoind.pid" || \
		brocoind -regtest -txindex -fallbackfee=0.00000253 -daemon

	# Wait for it to start.
	while ! brocoin-cli -regtest ping 2> /tmp/null; do echo "awaiting brocoind..." && sleep 1; done

	# Kick it out of initialblockdownload if necessary
	if brocoin-cli -regtest getblockchaininfo | grep -q 'initialblockdownload.*true'; then
		# Modern brocoind needs createwallet
		brocoin-cli -regtest createwallet default >/dev/null 2>&1
		brocoin-cli -regtest generatetoaddress 1 "$(brocoin-cli -regtest getnewaddress)" > /dev/null
	fi
	alias bt-cli='brocoin-cli -regtest'
...
stop_ln() {
	...
	test ! -f "$PATH_TO_BROCOIN/regtest/brocoind.pid" || \
		(kill "$(cat "$PATH_TO_BROCOIN/regtest/brocoind.pid")"; \
		rm "$PATH_TO_BROCOIN/regtest/brocoind.pid")
...
start_elem() {
	...
	test -f "$PATH_TO_ELEMENTS/liquid-regtest/brocoin.pid" || \
...
stop_elem() {
	...
	test ! -f "$PATH_TO_ELEMENTS/liquid-regtest/brocoind.pid" || \
		(kill "$(cat "$PATH_TO_ELEMENTS/liquid-regtest/brocoind.pid")"; \
		rm "$PATH_TO_ELEMENTS/liquid-regtest/brocoind.pid")
