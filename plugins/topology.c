...
{
	u64 costs = cost.millibroneess + risk.millibroneess; /* Raw: score */
			       ...
			       const struct route_hop *r)
{
	...
	json_add_amount_mbro_compat(js, r->amount, "mbronees", "amount_mbro");
					    ...
					    const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_req("mbronees", param_mbro, &mbro),
			      ...
			      int dirbits)
{
	...
	for (int dir = 0; dir < 2; dir++) {
		...
		json_add_num(response, "base_fee_millibronees", fee_base_mbro);
...
static const struct plugin_command commands[] = {
	{
		...
		"Show route to {id} for {mbronees}, using {riskfactor} and optional {cltv} (default 9). "
