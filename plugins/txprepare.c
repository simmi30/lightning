						 ...
						 bool is_withdraw)
{
	...
	if (txp->all_output_idx == -1)
		json_add_amount_bro_only(req->js, "bronees", txp->output_total);
	else
		json_add_string(req->js, "bronees", "all");
					    ...
					    const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   p_req("destination", param_brocoin_address,
			 &scriptpubkey),
		   p_req("bronees", param_bro_or_all, &amount),
		   p_opt("feerate", param_string, &feerate),
		   p_opt_def("minconf", param_number, &minconf, 1),
		   p_opt("utxos", param_outpoint_arr, &utxos),
		   NULL))
	...
	/* Convert destination/bronees into array as txprepare expects */
...
static const struct plugin_command commands[] = {
	...
	},
	{
		...
		"Send to {destination} {bronees} (or 'all') at optional {feerate} using utxos from {minconf} or {utxos}.",
		json_withdraw
	},
};
