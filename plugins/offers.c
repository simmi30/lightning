...
static void json_add_chains(struct json_stream *js,
			    const struct brocoin_blkid *chains)
{
				   ...
				   const struct blinded_payinfo *payinfo)
{
	...
	if (payinfo) {
		json_add_u32(js, "fee_base_mbro", payinfo->fee_base_mbro);
...
static void json_add_offer(struct json_stream *js, const struct tlv_offer *offer)
{
	...
	if (offer->currency) {
		...
		json_add_amount_mbro_only(js, "amount_mbro",
					  amount_mbro(*offer->amount));
			       ...
			       struct fallback_address **fallbacks)
{
	...
	if (tal_count(chains) != 0)
		...
		chain = chainparams_for_network("brocoin");
	...
	for (size_t i = 0; i < tal_count(fallbacks); i++) {
		...
		/* BOLT-offers #12:
		 * - for the brocoin chain, if the invoice specifies `fallbacks`:
