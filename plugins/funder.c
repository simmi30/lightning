		  ...
		  struct open_info *info)
{
	...
	req = jsonrpc_request_start(cmd->plugin, cmd,
				    ...
				    &psbt_fund_failed,
				    info);
	json_add_string(req->js, "bronees",
			type_to_string(tmpctx, struct amount_bro,
				       &info->our_funding));
		 ...
		 u64 **mod)
{
	...
	*mod = tal(cmd, u64);
	...
	if (err) {
		...
		**mod = bros.broneess; /* Raw: convert to u64 */
	}
...
static char *option_channel_base(const char *arg, struct funder_policy *policy)
{
	...
	if (!assign_overflow_u32(&policy->rates->channel_fee_max_base_mbro,
				amt.millibroneess)) /* Raw: conversion */
...
static char *amount_bro_or_u64_option(const char *arg, u64 *amt)
{
	...
	if (err) {
		...
		*amt = bros.broneess; /* Raw: convert to u64 */
	}
...
int main(int argc, char **argv)
{
	...
	plugin_main(argv, init, PLUGIN_RESTARTABLE, true,
		    ...
		    plugin_option("lease-fee-basis",
				  ...
				  " for leased funds (per 10,000 bronees.)",
