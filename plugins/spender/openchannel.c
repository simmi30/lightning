...
collect_sigs(struct multifundchannel_command *mfc)
{
	...
	for (size_t i = 0; i < tal_count(mfc->destinations); i++) {
		...
		assert(brocoin_txid_eq(mfc->txid, &dest_txid));
	}
...
openchannel_init_dest(struct multifundchannel_destination *dest)
{
	...
	if (amount_mbro_greater(dest->push_mbro, AMOUNT_MBRO(0)))
		plugin_log(cmd->plugin, LOG_INFORM,
			   "Using openchannel for %s open, "
			   "ignoring `push_mbro` of %s",
			   node_id_to_hexstr(tmpctx, &dest->id),
			   type_to_string(tmpctx, struct amount_mbro,
					  &dest->push_mbro));

	/* Request some bros from the peer! */
	if (!amount_bro_zero(dest->request_amt)) {
		json_add_string(req->js, "request_amt",
				fmt_amount_bro(tmpctx, dest->request_amt));
