							   ...
							   const jsmntok_t *tok)
{
	...
	const jsmntok_t *privtok = json_get_member(buffer, tok, "private"),
			...
			*tmbrotok =
			    json_get_member(buffer, tok, "total_mbro"),
			*smbrotok =
			    json_get_member(buffer, tok, "spendable_mbro");
	...
	if (privtok == NULL || privtok->type != JSMN_PRIMITIVE ||
	    ...
	    tmbrotok == NULL || tmbrotok->type != JSMN_STRING ||
	    smbrotok == NULL || smbrotok->type != JSMN_STRING)
		return NULL;
	...
	json_to_mbro(buffer, tmbrotok, &chan->total_mbro);
	json_to_mbro(buffer, smbrotok, &chan->spendable_mbro);
				      ...
				      const jsmntok_t *toks)
{
	...
	const jsmntok_t *amounttok = json_get_member(buffer, toks, "amount_mbro");
	...
	json_to_mbro(buffer, amounttok, &dst->amount);
