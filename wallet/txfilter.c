...
static size_t outpoint_hash(const struct brocoin_outpoint *out)
{
	struct siphash24_ctx ctx;
	siphash24_init(&ctx, siphash_seed());
...
static const struct brocoin_outpoint *outpoint_keyof(const struct brocoin_outpoint *out)
{
	return out;
}

HTABLE_DEFINE_TYPE(struct brocoin_outpoint, outpoint_keyof, outpoint_hash, brocoin_outpoint_eq,
		   outpointset);
...
bool txfilter_match(const struct txfilter *filter, const struct brocoin_tx *tx)
{
	for (size_t i = 0; i < tx->wtx->num_outputs; i++) {
		const u8 *oscript = brocoin_tx_output_get_script(tmpctx, tx, i);
