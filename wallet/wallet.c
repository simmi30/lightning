		     ...
		     struct channel *chan)
{
	...
	db_col_amount_bro(stmt, "funding_bronees", &funding_bro);
	db_col_amount_bro(stmt, "our_funding_bronees", &our_funding_bro);
				       ...
				       struct channel_config *cc)
{
	...
	const char *query = SQL(
	    "SELECT dust_limit_broneess, max_htlc_value_in_flight_mbro, "
	    "channel_reserve_broneess, htlc_minimum_mbro, to_self_delay, "
	...
	cc->id = id;
	db_col_amount_bro(stmt, "dust_limit_broneess", &cc->dust_limit);
	db_col_amount_mbro(stmt, "max_htlc_value_in_flight_mbro",
			   &cc->max_htlc_value_in_flight);
	db_col_amount_bro(stmt, "channel_reserve_broneess",
			  &cc->channel_reserve);
...
static struct channel *wallet_stmt2channel(struct wallet *w, struct db_stmt *stmt)
{
	...
	db_col_amount_bro(stmt, "funding_bronees", &funding_bro);
	db_col_amount_bro(stmt, "our_funding_bronees", &our_funding_bro);
	db_col_amount_mbro(stmt, "push_mbronees", &push_mbro);
	db_col_amount_mbro(stmt, "mbronees_local", &our_mbro);
	db_col_amount_mbro(stmt, "mbronees_to_us_min", &mbro_to_us_min);
	db_col_amount_mbro(stmt, "mbronees_to_us_max", &mbro_to_us_max);
	...
	chan = new_channel(peer, db_col_u64(stmt, "id"),
			   ...
			   &cid,
			   our_mbro,
			   mbro_to_us_min, /* mbronees_to_us_min */
			   mbro_to_us_max, /* mbronees_to_us_max */
			   db_col_psbt_to_tx(tmpctx, stmt, "last_tx"),
			   &last_sig,
				 ...
				 struct amount_mbro mbro)
{
	...
	case IN_OFFERED:
		query = SQL("UPDATE channels"
			    ...
			    "     , in_mbronees_offered = COALESCE(in_mbronees_offered, 0) + ?"
			    " WHERE id = ?;");
		break;
	case IN_FULLFILLED:
		query = SQL("UPDATE channels"
			    "   SET in_payments_fulfilled = COALESCE(in_payments_fulfilled, 0) + 1"
			    "     , in_mbronees_fulfilled = COALESCE(in_mbronees_fulfilled, 0) + ?"
			    " WHERE id = ?;");
		break;
	case OUT_OFFERED:
		query = SQL("UPDATE channels"
			    "   SET out_payments_offered = COALESCE(out_payments_offered, 0) + 1"
			    "     , out_mbronees_offered = COALESCE(out_mbronees_offered, 0) + ?"
			    " WHERE id = ?;");
		break;
	case OUT_FULLFILLED:
		query = SQL("UPDATE channels"
			    "   SET out_payments_fulfilled = COALESCE(out_payments_fulfilled, 0) + 1"
			    "     , out_mbronees_fulfilled = COALESCE(out_mbronees_fulfilled, 0) + ?"
			    " WHERE id = ?;");
		break;
	}
			       ...
			       struct channel_stats *stats)
{
	...
	stmt = db_prepare_v2(w->db, SQL(
				     ...
				     ",  in_mbronees_offered,  in_mbronees_fulfilled"
				     ", out_payments_offered, out_payments_fulfilled"
				     ", out_mbronees_offered, out_mbronees_fulfilled"
				     "  FROM channels"
				     " WHERE id = ?"));
	db_bind_u64(stmt, 0, id);
	db_query_prepared(stmt);
	...
	db_col_amount_mbro_or_default(stmt, "in_mbronees_offered",
				      &stats->in_mbronees_offered,
				      AMOUNT_MBRO(0));
	db_col_amount_mbro_or_default(stmt, "in_mbronees_fulfilled",
				      &stats->in_mbronees_fulfilled,
				      AMOUNT_MBRO(0));
	...
	db_col_amount_mbro_or_default(stmt, "out_mbronees_offered",
				      &stats->out_mbronees_offered,
				      AMOUNT_MBRO(0));
	db_col_amount_mbro_or_default(stmt, "out_mbronees_fulfilled",
				      &stats->out_mbronees_fulfilled,
				      AMOUNT_MBRO(0));
	tal_free(stmt);
}
...
void wallet_channel_save(struct wallet *w, struct channel *chan)
{
	...
	stmt = db_prepare_v2(w->db, SQL("UPDATE channels SET"
					...
					"  mbronees_to_us_min=?," // 25
					"  mbronees_to_us_max=?," // 26
					"  feerate_base=?," // 27
					"  feerate_ppm=?," // 28
				...
				struct db_stmt *stmt, struct htlc_in *in)
{
	...
	db_col_amount_mbro(stmt, "mbronees", &in->mbro);
				 ...
				 struct htlc_in_map *unconnected_htlcs_in)
{
	...
	db_col_amount_mbro(stmt, "mbronees", &out->mbro);
...
bool wallet_invoice_resolve(struct wallet *wallet,
			    struct invoice invoice,
			    struct amount_mbro mbronees_received)
{
	return invoices_resolve(wallet->invoices, invoice, mbronees_received);
}
			  ...
			  struct wallet_payment *payment TAKES)
{
	...
	db_bind_amount_mbro(stmt, 3, &payment->mbronees);
	...
	db_bind_amount_mbro(stmt, 8, &payment->mbronees_sent);
						  ...
						  struct db_stmt *stmt)
{
	...
	db_col_amount_mbro(stmt, "mbronees", &payment->mbronees);
	...
	db_col_amount_mbro(stmt, "mbronees_sent", &payment->mbronees_sent);
					  ...
					  const struct short_channel_id *scid)
{
	...
	db_col_amount_bro(stmt, "broneess", &op->bro);
	tal_free(stmt);
...
struct amount_mbro wallet_total_forward_fees(struct wallet *w)
{
	...
	stmt = db_prepare_v2(w->db, SQL("SELECT"
					" CAST(COALESCE(SUM(in_mbronees - out_mbronees), 0) AS BIGINT)"
	...
	db_col_amount_mbro(stmt, "CAST(COALESCE(SUM(in_mbronees - out_mbronees), 0) AS BIGINT)", &total);
	tal_free(stmt);
						       ...
						       const struct short_channel_id *chan_out)
{
	...
	for (count=0; db_step(stmt); count++) {
		...
		db_col_amount_mbro(stmt, "in_mbronees", &cur->mbro_in);

		if (!db_col_is_null(stmt, "out_mbronees")) {
			db_col_amount_mbro(stmt, "out_mbronees", &cur->mbro_out);
