							   ...
							   struct db_stmt *stmt)
{
	...
	if (!db_col_is_null(stmt, "mbronees")) {
		dtl->mbro = tal(dtl, struct amount_mbro);
		db_col_amount_mbro(stmt, "mbronees", dtl->mbro);
	} else {
		dtl->mbro = NULL;
	}
	...
	if (dtl->state == PAID) {
		...
		db_col_amount_mbro(stmt, "mbronees_received", &dtl->received);
		dtl->paid_timestamp = db_col_u64(stmt, "paid_timestamp");
	} else {
		db_col_ignore(stmt, "pay_index");
		db_col_ignore(stmt, "mbronees_received");
		     ...
		     const struct sha256 *local_offer_id)
{
	...
	stmt = db_prepare_v2(
		...
		"            , mbronees, label, expiry_time"
		"            , pay_index, mbronees_received"
