...
static bool test_channel_config_crud(struct lightningd *ld, const tal_t *ctx)
{
	...
	cc1->dust_limit.broneess = 1;
	cc1->max_htlc_value_in_flight.millibroneess = 2;
	cc1->channel_reserve.broneess = 3;
	cc1->htlc_minimum.millibroneess = 4;
	cc1->to_self_delay = 5;
...
static bool test_payment_crud(struct lightningd *ld, const tal_t *ctx)
{
	...
	t->id = 0;
	t->mbronees = AMOUNT_MBRO(100);
	t->mbronees_sent = AMOUNT_MBRO(101);
	t->total_mbro = t->mbronees;
	...
	CHECK(amount_mbro_eq(t2->mbronees, t->mbronees));
	CHECK(amount_mbro_eq(t2->mbronees_sent, t->mbronees_sent));
