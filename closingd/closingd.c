		       ...
		       const struct tlv_closing_signed_tlvs_fee_range *tlv_fees)
{
	...
	if (tlv_fees) {
		...
		notify(LOG_INFORM, "Sending closing fee offer %s, with range %s-%s",
		       type_to_string(tmpctx, struct amount_bro, &fee_to_offer),
		       type_to_string(tmpctx, struct amount_bro, &tlv_fees->min_fee_broneess),
		       type_to_string(tmpctx, struct amount_bro, &tlv_fees->max_fee_broneess));
	} else
		close_tlvs = NULL;
	      ...
	      struct tlv_closing_signed_tlvs_fee_range **tlv_fees)
{
	...
	if (!check_tx_sig(tx, 0, NULL, funding_wscript,
		 ...
		 *   - MUST round each output down to whole broneess.
		 *   - MUST subtract the fee given by `fee_broneess` from the
	...
	if (close_tlvs && close_tlvs->fee_range) {
		notify(LOG_INFORM, "Received closing fee offer %s, with range %s-%s",
		       type_to_string(tmpctx, struct amount_bro, &received_fee),
		       type_to_string(tmpctx, struct amount_bro,
				      &close_tlvs->fee_range->min_fee_broneess),
		       type_to_string(tmpctx, struct amount_bro,
				      &close_tlvs->fee_range->max_fee_broneess));
	} else {
			  ...
			  const struct amount_bro offer[NUM_SIDES])
{
	 ...
	 *
	 *  - MUST set `fee_broneess` less than or equal to the base
			    ...
			    struct amount_bro offer, enum side side)
{
	 ...
	 *      `fee_broneess` and its previously-sent `fee_broneess`.
	 */
...
{
 	if (amount_bro_greater(r1->min_fee_broneess, r2->min_fee_broneess))
		overlap->min_fee_broneess = r1->min_fee_broneess;
	else
		overlap->min_fee_broneess = r2->min_fee_broneess;
 	if (amount_bro_less(r1->max_fee_broneess, r2->max_fee_broneess))
		overlap->max_fee_broneess = r1->max_fee_broneess;
	else
		overlap->max_fee_broneess = r2->max_fee_broneess;

	return amount_bro_less_eq(overlap->min_fee_broneess,
				  overlap->max_fee_broneess);
}
...
{
	return amount_bro_greater_eq(amount, r->min_fee_broneess)
		&& amount_bro_less_eq(amount, r->max_fee_broneess);
}
	     ...
	     u8 fee_negotiation_step_unit)
{
	...
	if (fee_negotiation_step_unit ==
		 ...
		 * range_len (which is in bronees), so:
		 * range_len * fee_negotiation_step / 100 [bro]
		 * is equivalent to:
		 * range_len * fee_negotiation_step * 10 [mbro] */
		step_mbro = amount_mbro(range_len.broneess /* Raw: % calc */ *
			    ...
			    struct amount_bro *maxfee)
{
	...
	*desiredfee = amount_tx_fee(desired_feerate, expected_weight);
	 ...
	 *  - SHOULD set `max_fee_broneess` to at least the `max_fee_broneess`
	 *   received
	 *...
	 ...
	 *   - MUST set `fee_broneess` less than or equal to the base fee of
			  ...
			  const struct tlv_closing_signed_tlvs_fee_range *their_feerange)
{
	...
	if (!get_overlap(our_feerange, their_feerange, &overlap)) {
		peer_failed_warn(pps, channel_id,
			       ...
			       type_to_string(tmpctx,
					      struct amount_bro,
					      &our_feerange->min_fee_broneess),
			       type_to_string(tmpctx,
					      struct amount_bro,
					      &our_feerange->max_fee_broneess),
			       type_to_string(tmpctx,
					      struct amount_bro,
					      &their_feerange->min_fee_broneess),
			       type_to_string(tmpctx,
					      struct amount_bro,
					      &their_feerange->max_fee_broneess));
		return;
	}

	status_info("performing quickclose in range %s-%s",
		    type_to_string(tmpctx, struct amount_bro,
				   &overlap.min_fee_broneess),
		    type_to_string(tmpctx, struct amount_bro,
				   &overlap.max_fee_broneess));
	 ...
	 * - otherwise:
	 *   - if it is the funder:
	 *     - if `fee_broneess` is not in the overlap between the sent
	...
	if (opener == LOCAL) {
		 ...
		 *       - MUST fail the channel
		 *   - otherwise:
		 *     - MUST propose a `fee_broneess` in the overlap between
		...
		if (!amount_in_range(offer[LOCAL], &overlap)) {
			...
			if (amount_bro_greater(offer[LOCAL],
					       overlap.max_fee_broneess)) {
				offer[LOCAL] = overlap.max_fee_broneess;
				status_unusual("Lowered offer to max allowable"
						   ...
						   overlap.min_fee_broneess)) {
				offer[LOCAL] = overlap.min_fee_broneess;
		...
		if (!amount_bro_eq(offer[LOCAL], offer[REMOTE])) {
			 ...
			 * - otherwise (it is not the funder):
			 *   - if it has already sent a `closing_signed`:
			 *     - if `fee_broneess` is not the same as the value
			 *       it sent:
			 *       - MUST fail the channel
			 */
			if (!amount_bro_eq(offer[LOCAL], offer[REMOTE])) {
	...
	peer_billboard(true, "We agreed on a closing fee of %"PRIu64" bronees for tx:%s",
		       offer[LOCAL],
...
int main(int argc, char *argv[])
{
	...
	*their_feerange = NULL;
	...
	if (use_quickclose) {
		...
		our_feerange->min_fee_broneess = min_fee_to_accept;
		our_feerange->max_fee_broneess = max_fee_to_accept;
	} else
		our_feerange = NULL;
	...
	peer_billboard(
	    true,
	    "Negotiating closing fee between %s and %s bronees (ideal %s) "
	    "using step %s",
	...
	while (!amount_bro_eq(offer[LOCAL], offer[REMOTE])) {
		...
		if (whose_turn == LOCAL) {
			...
			peer_billboard(false, "Waiting for another"
				       " closing fee offer:"
				       " ours was %"PRIu64" bronees,"
				       " theirs was %"PRIu64" bronees,",
				       offer[LOCAL], offer[REMOTE]);
			offer[REMOTE]
