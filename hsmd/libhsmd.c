...
{
	bool is_outbound;
	struct amount_bro channel_value;
	struct amount_mbro push_value;
	...
	assert(amount_bro_greater(channel_value, AMOUNT_BRO(0)));
	assert(amount_bro_to_mbro(&value_mbro, channel_value));
	assert(amount_mbro_less_eq(push_value, value_mbro));
 ...
 * don't fill it in. */
static void brocoin_key(struct privkey *privkey, struct pubkey *pubkey,
			u32 index)
{
	struct ext_key ext;
...
/* This gets the brocoin private key needed to spend from our wallet */
			     ...
			     const struct utxo *utxo)
{
	if (utxo->close_info != NULL) {
		...
		brocoin_key(privkey, pubkey, utxo->keyindex);
	}
}
					      ...
					      const u8 *msg_in)
{
	...
	u32 lease_expiry, channel_fee_base_max_mbro;
 ...
 * (to prove it's from us), and one from the brocoin key used to create the
...
static u8 *handle_cannouncement_sig(struct hsmd_client *c, const u8 *msg_in)
{
	...
	size_t offset = 2 + 256;
	struct privkey node_pkey;
	secp256k1_ecdsa_signature node_sig, brocoin_sig;
	...
	sign_hash(&funding_privkey, &hash, &brocoin_sig);
...
static u8 *handle_channel_update_sig(struct hsmd_client *c, const u8 *msg_in)
{
	...
	u32 timestamp, fee_base_mbro, fee_proportional_mill;
	...
	cu = towire_channel_update_option_channel_htlc_max(tmpctx, &sig, &chain_hash,
				   ...
				   fee_base_mbro, fee_proportional_mill,
				   htlc_maximum);
...
static u8 *handle_sign_commitment_tx(struct hsmd_client *c, const u8 *msg_in)
{
	...
	/*~ Brocoin signatures cover the (part of) the script they're
	      ...
	      struct bip32_key_version bip32_key_version)
{
	...
	/* Fill in the BIP32 tree for brocoin addresses. */
	 ...
	 * as purpose.
	 */
	/* Clearly, we should use 9443, the unicode point for lightning! */
	if (bip32_key_from_parent(&master_extkey,
				  BIP32_INITIAL_HARDENED_CHILD|9443,
