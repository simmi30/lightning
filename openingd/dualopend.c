				   ...
				   struct psbt_changeset *set)
{
	...
	if (tal_count(set->added_outs) != 0) {
		...
		msg = towire_tx_add_output(ctx, cid, serial_id,
					   bros.broneess, /* Raw: wire interface */
					   script);
			...
			enum tx_role our_role)
{
	 ...
	 * channel balance (`open_channel2`.`funding_broneess` +
	 * `accept_channel2`.`funding_broneess`) rounded down to the
	 * nearest whole bronees or the `dust_limit_broneess`, whichever is
	 * greater.
	 */
			    ...
			    u32 feerate_per_kw_funding)
{
	...
	if (find_txout(psbt,
		 ...
		 *       sum of `open_channel2`.`funding_broneess`
		 *       and `accept_channel2`. `funding_broneess`
		 */
	 ...
	 *  - MUST fail the negotiation if:
	 *   ...
	 *   - the peer's total input broneess is less than their outputs
	 */
				 ...
				 u32 blockheight)
{
	 ...
	 *     (in bronees) it will charge for the `funding_broneess`
	 *   - MUST set `funding_fee_proportional_basis` to the amount
	 *     (in thousandths of bronees) it will charge per `funding_bronees`
	 ...
	 *     (in millibronees) it will charge for any HTLC on this channel
	 *     during the funding period.
	 *   - MUST set `channel_fee_proportional_basis_max` to the amount
	 *     (in thousandths of a bronees) it will charge per transferred
	 *     bronees during the funding period.
	 */
...
static void accepter_start(struct state *state, const u8 *oc2_msg)
{
	 ...
	 * The receiving node MUST fail the channel if:
	 *...
	 * - `funding_broneess` is greater than or equal to 2^24 and the receiver does not support
	...
	if (!feature_negotiated(state->our_features, state->their_features,
		...
		negotiation_failed(state,
				   "opener's funding_broneess %s too large",
				   type_to_string(tmpctx, struct amount_bro,
						  &tx_state->opener_funding));
		return;
	}
	 ...
	 *   ...
	 *   - MUST set `funding_broneess` to a value greater than 0mbro
	 */
	...
	if (tx_state->rates) {
		 ...
		 * in a channel, in addition to the `funding_bronees`
		 * that they are contributing. The channel initiator
		 * must contribute enough funds to cover
		 * `open_channel2`.`funding_broneess`, the lease fee,
	...
	if (!feature_negotiated(state->our_features, state->their_features,
		...
		negotiation_failed(state, "total funding_broneess %s too large",
				   type_to_string(tmpctx, struct amount_bro,
						  &total));
		return;
	}
...
static void opener_start(struct state *state, u8 *msg)
{
	...
	if (!amount_bro_zero(requested_bros)) {
		...
		open_tlv->request_funds->requested_bros =
			requested_bros.broneess; /* Raw: struct -> wire */
	 ...
	 * The receiving node MUST fail the channel if:
	 *...
	 * - `funding_broneess` is greater than or equal to 2^24 and
	...
	if (!feature_negotiated(state->our_features, state->their_features,
	    ...
	    && amount_bro_greater(total, chainparams->max_funding)) {
		negotiation_failed(state,
				   "total funding_broneess %s too large",
				   type_to_string(tmpctx, struct amount_bro,
						  &total));
		return;
	}
...
static void rbf_local_start(struct state *state, u8 *msg)
{
	...
	if (!feature_negotiated(state->our_features, state->their_features,
				OPT_LARGE_CHANNELS)
	    && amount_bro_greater(total, chainparams->max_funding)) {
		open_err_warn(state, "Total funding_broneess %s too large",
			      type_to_string(tmpctx,
					     struct amount_bro,
					     &total));
		goto free_rbf_ctx;
