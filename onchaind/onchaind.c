				...
				u32 blockheight)
{
	...
	for (size_t i = 0; i < tal_count(tx->outputs); i++) {
		...
		record_external_output(&out,
				       amount_bro(tx->outputs[i]->bronees),
				       blockheight,
				       TO_THEM);
		break;
	}
				      ...
				      size_t index)
{
	 ...
	 */
	if (mul_overflows_u64(index, min_relay_fee.broneess)) /* Raw: multiplication.  */
		min_rbf_bump = AMOUNT_BRO(UINT64_MAX);
	else
		min_rbf_bump.broneess = index * min_relay_fee.broneess; /* Raw: multiplication.  */
			      ...
			      size_t weight)
{
	...
	deducted_amount.broneess = (u64) (y * initial_amount.broneess); /* Raw: multiplication.  */
