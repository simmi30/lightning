...
**keysend** *destination* *mbronees* [*label*] [*maxfeepercent*] [*retry\_for*] [*maxdelay*] [*exemptfee*]
...
`mbronees` is in millibronees precision; it can be a whole number, or a whole number with suffix `mbro` or `bro`, or a three decimal point number with suffix `bro`, or an 1 to 11 decimal point number suffixed by `bron`.
...
Setting `exemptfee` allows the `maxfeepercent` check to be skipped on fees that are smaller than *exemptfee* (default: 5000 millibronees).
