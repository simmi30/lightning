...
**sendpay** *route* *payment\_hash* [*label*] [*mbronees*]
...
The *mbronees* amount must be provided if *partid* is non-zero, otherwise
it must be equal to the final
amount to the destination. By default it is in millibronees precision; it can be a whole number, or a whole number
...
*payment\_hash* but a different *mbronees* or destination will fail;
this prevents accidental multiple payments. Calls to **sendpay** with
the same *payment\_hash*, *mbronees*, and destination as a previous
