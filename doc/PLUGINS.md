...
(in millibroneess), the funding transaction id, and a boolean indicating
...
```json
{
  "forward_event": {
    ...
    "in_channel": "103x2x1",
    "out_channel": "103x1x1",
    "in_mbronees": 100001001,
    "in_mbro": "100001001mbro",
    "out_mbronees": 100000000,
 ...
 - The status includes `offered`, `settled`, `failed` and `local_failed`,
   ...
   - If payment fails locally(like failing to resolve locally) or the
     ...
     - If `local_failed` is set before `offered`, this
       ...
       sets the fields of `out_mbronees`, `out_mbro`,`fee` and `out_channel`
       as 0;
...
`credit` and `debit` are millibronees denominated amounts of the fund movement. A
