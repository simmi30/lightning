...
**invoice** *mbronees* *label* *description* [*expiry*]
...
given amount of milli-bronees: it returns a unique token which another
...
The *mbronees* parameter can be the string "any", which creates an
invoice that can be paid with any amount. Otherwise it is a positive value in
millibronees precision; it can be a whole number, or a whole number
...
peer's balance minus their reserves is at least *mbronees*, state is
