...
static bool contains_common_chain(struct brocoin_blkid *chains)
{
	for (size_t i = 0; i < tal_count(chains); i++) {
		if (brocoin_blkid_eq(&chains[i], &chainparams->genesis_blockhash))
			return true;
	}
	return false;
}
				      ...
				      bool incoming)
{
	...
	tlvs->networks = tal_dup_arr(tlvs, struct brocoin_blkid,
