...
{
	assert(chan->bro.broneess == (unsigned long)chan); /* Raw: dev-hack */
}
#endif
...
#if DEVELOPER
	chan->bro.broneess = (unsigned long)chan; /* Raw: dev-hack */
#endif
	tal_free(chan);
}
				...
				bool ignore_timestamp)
{
	...
	if (message_flags & ROUTING_OPT_HTLC_MAX_MBRO) {
		 ...
		 * than the total available channel broneess */
