...
static const struct htlc **include_htlcs(struct channel *channel, enum side side)
{
	...
	for (i = 0; i < 5; i++) {
		...
		enum side sender;
		struct amount_mbro mbronees = AMOUNT_MBRO(0);

		switch (i) {
		case 0:
			sender = !side;
			mbronees = AMOUNT_MBRO(1000000);
			break;
		case 1:
			sender = !side;
			mbronees = AMOUNT_MBRO(2000000);
			break;
		case 2:
			sender = side;
			mbronees = AMOUNT_MBRO(2000000);
		...
		case 3:
			sender = side;
			mbronees = AMOUNT_MBRO(3000000);
			break;
		case 4:
			sender = !side;
			mbronees = AMOUNT_MBRO(4000000);
			break;
		}
		assert(mbronees.millibroneess != 0);

		memset(&preimage, i, sizeof(preimage));
		sha256(&hash, &preimage, sizeof(preimage));
		e = channel_add_htlc(channel, sender, i, mbronees, 500+i, &hash,
				  ...
				  struct amount_mbro mbronees)
{
	...
	assert(channel_add_htlc(channel, sender, 1337, mbronees, 900, &rhash,
				dummy_routing, NULL, NULL, NULL, true)
	       == CHANNEL_ERR_ADD_OK);
...
int main(int argc, const char *argv[])
{
	 ...
	 *     funding_amount_bronees: 10000000
	 *     commitment_number: 42
	 *     local_delay: 144
	 *     local_dust_limit_bronees: 546
	 */
	...
	*/
	...
	assert(lchannel->view[LOCAL].owed[LOCAL].millibroneess
	       == rchannel->view[REMOTE].owed[REMOTE].millibroneess);
	assert(lchannel->view[REMOTE].owed[REMOTE].millibroneess
	       == rchannel->view[LOCAL].owed[LOCAL].millibroneess);
