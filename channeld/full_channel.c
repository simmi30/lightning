...
{
	balance->mbro = mbro.millibroneess; /* Raw: balance */
			     ...
			     enum side side)
{
	if (htlc_owner(htlc) == side)
		balance->mbro -= htlc->amount.millibroneess; /* Raw: balance */
}
...
static bool sum_offered_mbroneess(struct amount_mbro *total,
				     ...
				     bool err_immediate_failures)
{
	...
	if (!sum_offered_mbroneess(&committed_mbro,
				   committed, htlc_owner(htlc))
	    || !sum_offered_mbroneess(&removing_mbro,
				      removing, htlc_owner(htlc))
	    || !sum_offered_mbroneess(&adding_mbro,
	...
	if (amount_mbro_greater(htlc_dust_amt,
		 ...
		 *   counterparty's `dust_limit_broneess` plus the HTLC-timeout fee
...
u32 approx_max_feerate(const struct channel *channel)
{
	...
	return avail.broneess / weight * 1000; /* Raw: once-off reverse feerate*/
}
