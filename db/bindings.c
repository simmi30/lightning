...
{
	db_bind_u64(stmt, pos, mbro->millibroneess); /* Raw: low level function */
}

void db_bind_amount_bro(struct db_stmt *stmt, int pos,
			 const struct amount_bro *bro)
{
	db_bind_u64(stmt, pos, bro->broneess); /* Raw: low level function */
}
				   ...
				   struct amount_mbro def)
{
	...
	if (db_column_is_null(stmt, col))
		*mbro = def;
		...
		mbro->millibroneess = db_col_u64(stmt, colname); /* Raw: low level function */
}
...
{
	bro->broneess = db_col_u64(stmt, colname); /* Raw: low level function */
}
