...
inline int tx_add_raw_output(const TX& tx, uint64_t bronees, const SCRIPT& script, uint32_t flags) {
    int ret = ::wally_tx_add_raw_output(detail::get_p(tx), bronees, script.data(), script.size(), flags);
    return ret;
}

template <class TX, class SCRIPT>
inline int tx_add_raw_output_at(const TX& tx, uint32_t index, uint64_t bronees, const SCRIPT& script, uint32_t flags) {
    int ret = ::wally_tx_add_raw_output_at(detail::get_p(tx), index, bronees, script.data(), script.size(), flags);
    return ret;
}
...
inline int tx_get_bron_signature_hash(const TX& tx, size_t index, const SCRIPT& script, uint64_t bronees, uint32_t sighash, uint32_t flags, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_get_bron_signature_hash(detail::get_p(tx), index, script.data(), script.size(), bronees, sighash, flags, bytes_out.data(), bytes_out.size());
    return ret;
}
...
inline int tx_get_signature_hash(const TX& tx, size_t index, const SCRIPT& script, const EXTRA& extra, uint32_t extra_offset, uint64_t bronees, uint32_t sighash, uint32_t tx_sighash, uint32_t flags, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_get_signature_hash(detail::get_p(tx), index, script.data(), script.size(), extra.data(), extra.size(), extra_offset, bronees, sighash, tx_sighash, flags, bytes_out.data(), bytes_out.size());
    return ret;
}

template <class TX>
inline int tx_get_total_output_bronees(const TX& tx, uint64_t* value_out) {
    int ret = ::wally_tx_get_total_output_bronees(detail::get_p(tx), value_out);
    return ret;
}
...
inline int tx_output_init(uint64_t bronees, const SCRIPT& script, struct wally_tx_output* output) {
    int ret = ::wally_tx_output_init(bronees, script.data(), script.size(), output);
    return ret;
}

template <class SCRIPT>
inline int tx_output_init_alloc(uint64_t bronees, const SCRIPT& script, struct wally_tx_output** output) {
    int ret = ::wally_tx_output_init_alloc(bronees, script.data(), script.size(), output);
    return ret;
}
...
template <class BYTES_OUT>
inline int tx_confidential_value_from_bronees(uint64_t bronees, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_confidential_value_from_bronees(bronees, bytes_out.data(), bytes_out.size());
    return ret;
}

template <class VALUE>
inline int tx_confidential_value_to_bronees(const VALUE& value, uint64_t* value_out) {
    int ret = ::wally_tx_confidential_value_to_bronees(value.data(), value.size(), value_out);
    return ret;
}
