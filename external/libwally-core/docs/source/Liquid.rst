...
funds. We'll send 1.1 BRON to our confidential address and save the raw
...
transactions differ from standard brocoin transaction in that the fee is
...
Finally the fee output must be explicitly added (unlike standard Brocoin
transactions where the fee is implicit). The fee is always payable in
the brocoin asset. The wallet logic will determine the fee amount.
