...
#define MSG_ALL_FLAGS (BROCOIN_MESSAGE_FLAG_HASH)

static const char MSG_PREFIX[] = "\x18" "Brocoin Signed Message:\n";
...
int wally_format_brocoin_message(const unsigned char *bytes, size_t bytes_len,
                                 uint32_t flags,
                                 unsigned char *bytes_out, size_t len,
                                 size_t *written)
{
    unsigned char buf[256], *msg_buf = bytes_out, *out;
    const bool do_hash = (flags & BROCOIN_MESSAGE_FLAG_HASH);
    size_t msg_len;

    if (written)
        *written = 0;

    if (!bytes || !bytes_len || bytes_len > BROCOIN_MESSAGE_MAX_LEN ||
