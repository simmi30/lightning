...
FUNCS = [
    ...
    ('wally_format_brocoin_message', F([
        'const_bytes[message]', 'uint32_t[flags]',
    'out_bytes_sized'
    ], out_size='_arguments[1] & {} ? {} : 2 + "Brocoin Signed Message:".length + _arguments[0].length + (_arguments[0].length < 253 ? 1 : 3)'.format(
        BROCOIN_MESSAGE_FLAG_HASH, SHA256_LEN))),
]
FUNCS_NODE = [
    # Assets:
