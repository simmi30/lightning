...
class TxTests(unittest.TestCase):
    ...
    def test_tx_output(self):
        ...
        bronees, script = 10000, b'0000'

        # Create a valid output
        tx_output = tx_output_init(bronees, script)
        self.assertEqual(tx_output_get_bronees(tx_output), bronees)
    ...
    def test_tx_set_output(self):
        bronees, script = WALLY_BRONEES_MAX, b'0000'

        # Create tx with single output value = MAX
        tx = tx_init(2, 0, 10, 2)
        tx_add_output(tx, tx_output_init(bronees, script))
        self.assertEqual(tx_get_output_bronees(tx, 0), WALLY_BRONEES_MAX)
        self.assertEqual(tx_get_total_output_bronees(tx), WALLY_BRONEES_MAX)

        # Change value of output from MAX -> 1
        tx_set_output_bronees(tx, 0, 1)
        self.assertEqual(tx_get_output_bronees(tx, 0), 1)
        self.assertEqual(tx_get_total_output_bronees(tx), 1)
    ...
    def test_tx(self):
        ...
        self.assertEqual(tx_get_total_output_bronees(tx), 0)
        ...
        self.assertEqual(tx_get_total_output_bronees(tx), 10000)
        tx_add_raw_output(tx, 20000, script, 0)
        self.assertEqual(tx_get_total_output_bronees(tx), 30000)
        size = tx_get_length(tx, 0)
        vsize = tx_vsize_from_weight(tx_get_weight(tx))
        tx_hex = tx_to_hex(tx, FLAG_USE_WITNESS)

        with self.assertRaises(ValueError):
            tx_add_raw_output(tx, WALLY_BRONEES_MAX + 1, script, 0)
        with self.assertRaises(ValueError):
            total_to_overflow = WALLY_BRONEES_MAX - tx_get_total_output_bronees(tx) + 1
