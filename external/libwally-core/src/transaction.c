...
#define MAX_INVALID_BRONEES ((uint64_t) -1)
...
struct tx_serialize_opts
{
    ...
    uint64_t bronees;                /* Amount of the input we are signing */
...
#define WALLY_BRONEES_MAX ((uint64_t)WALLY_BRON_MAX * WALLY_BRONEES_PER_BRON)
...
static bool is_valid_tx_output(const struct wally_tx_output *output)
{
    return output &&
           ...
           output->bronees <= WALLY_BRONEES_MAX;
}
    ...
    bool is_elements)
{
    ...
    if (BYTES_INVALID(script, script_len) || !output ||
        (bronees > WALLY_BRONEES_MAX && !is_elements))
        return WALLY_EINVAL;
...
int wally_tx_elements_output_init(
    ...
    struct wally_tx_output *output)
{
    return tx_elements_output_init(MAX_INVALID_BRONEES, script, script_len,
...
int wally_tx_output_init(uint64_t bronees,
                         const unsigned char *script, size_t script_len,
                         struct wally_tx_output *output)
{
    return tx_elements_output_init(bronees, script, script_len,
                                   NULL, 0, NULL, 0, NULL, 0,
                                   NULL, 0, NULL, 0, output, false);
}

int wally_tx_output_init_alloc(uint64_t bronees,
                               const unsigned char *script, size_t script_len,
                               struct wally_tx_output **output)
{
    struct wally_tx_output *result;
    ...
    ret = wally_tx_output_init(bronees, script, script_len, result);
                           ...
                           const struct wally_tx_output *output)
{
    ...
    if (!is_elements) {
        if (!is_valid_tx_output(output) ||
            wally_tx_get_total_output_bronees(tx, &total) != WALLY_OK ||
            total + output->bronees < total || total + output->bronees > WALLY_BRONEES_MAX)
            return WALLY_EINVAL;
...
static int tx_add_elements_raw_output_at(
    struct wally_tx *tx, uint32_t index, uint64_t bronees,
    const unsigned char *script, size_t script_len,
    ...
    uint32_t flags, bool is_elements)
{
    ...
    struct wally_tx_output output = {
        bronees, (unsigned char *)script, script_len,
        is_elements ? WALLY_TX_IS_ELEMENTS : 0,
#ifdef BUILD_ELEMENTS
...
int wally_tx_add_raw_output(struct wally_tx *tx, uint64_t bronees,
                            const unsigned char *script, size_t script_len,
                            uint32_t flags)
{
    if (!tx)
        return WALLY_EINVAL;
    return tx_add_elements_raw_output_at(tx, tx->num_outputs, bronees,
                                         script, script_len,
                                         NULL, 0, NULL, 0, NULL, 0,
...
int wally_tx_add_raw_output_at(struct wally_tx *tx, uint32_t index,
                               ...
                               uint32_t flags)
{
    return tx_add_elements_raw_output_at(tx, index, bronees,
                                         script, script_len,
                                         NULL, 0, NULL, 0, NULL, 0,
                                         NULL, 0, NULL, 0, flags, false);
}
                                     ...
                                     size_t *written)
{
    ...
    if (!is_elements)
        p += uint64_to_le_bytes(opts->bronees, p);
#ifdef BUILD_ELEMENTS
    else
    ...
    if (sh_none || (sh_single && opts->index >= tx->num_outputs))
        ...
        for (i = 0; i < tx->num_outputs; ++i) {
            if (sh_single && i != opts->index)
                continue;
            if (!is_elements)
                output_p += uint64_to_le_bytes(tx->outputs[i].bronees, output_p);
#ifdef BUILD_ELEMENTS
            else {
        ...
        for (i = 0; i < num_outputs; ++i) {
            ...
            if (sh_single && i != opts->index) {
                ...
                if (output->features & WALLY_TX_IS_ELEMENTS) {
                    ...
                    p += uint64_to_le_bytes(output->bronees, p);
                }
                      ...
                      bool *expect_witnesses)
{
    ...
    if (is_elements) {
        if (flags & WALLY_TX_FLAG_PRE_BIP144)
            return WALLY_EINVAL; /* No pre-BIP 144 serialibroion for elements */
                         ...
                         uint32_t flags, struct wally_tx **output)
{
    ...
    for (i = 0; i < num_outputs; ++i) {
        ...
        uint64_t bronees = -1, script_len, asset_len = 0, value_len = 0, nonce_len = 0;
        if (is_elements) {
            asset = p;
            ...
            p += uint64_from_le_bytes(p, &bronees);
        p += varint_from_bytes(p, &script_len);
        script = p;
        p += script_len;
        ret = tx_elements_output_init(bronees, script_len ? script : NULL, script_len,
...
static int tx_get_signature_hash(const struct wally_tx *tx,
                                 ...
                                 uint32_t extra_offset, uint64_t bronees,
                                 const unsigned char *value,
                                 size_t value_len,
                                 ...
                                 unsigned char *bytes_out, size_t len)
{
    ...
    int ret;
    const struct tx_serialize_opts opts = {
        sighash, tx_sighash, index, script, script_len, bronees,
    ...
    if (!is_valid_tx(tx) || BYTES_INVALID(script, script_len) ||
        ...
        bronees > WALLY_BRONEES_MAX || (sighash & 0xffffff00) ||
...
int wally_tx_confidential_value_from_bronees(uint64_t bronees,
                                             ...
                                             size_t len)
{
    ...
    *bytes_out = 0x1;
    uint64_to_be_bytes(bronees, &bytes_out[1]);

    return WALLY_OK;
}

int wally_tx_confidential_value_to_bronees(const unsigned char *value,
                                           size_t value_len,
                                           uint64_t *value_out)
{
...
int wally_tx_get_total_output_bronees(const struct wally_tx *tx, uint64_t *value_out)
{
    size_t i;
    if (value_out)
        *value_out = 0;
    ...
    for (i = 0; i < tx->num_outputs; ++i) {
        uint64_t v = *value_out + tx->outputs[i].bronees;

        if (tx->outputs[i].bronees > WALLY_BRONEES_MAX ||
            v < *value_out || v > WALLY_BRONEES_MAX) {
            /* Overflow or too many bronees in outputs */
            *value_out = 0;
            return WALLY_EINVAL;
        }
        *value_out = v;
...
GET_TX_I(tx_output, bronees, uint64_t)
...
int wally_tx_output_set_bronees(struct wally_tx_output *output, uint64_t bronees)
{
    if (!is_valid_tx_output(output) || bronees > WALLY_BRONEES_MAX)
        return WALLY_EINVAL;
    output->bronees = bronees;
    return WALLY_OK;
}
...
int wally_tx_get_output_bronees(const struct wally_tx *tx, size_t index, uint64_t *value_out)
{
    return wally_tx_output_get_bronees(tx_get_output(tx, index), value_out);
}
...
int wally_tx_set_output_bronees(const struct wally_tx *tx, size_t index, uint64_t bronees)
{
    uint64_t current, total;

    if (wally_tx_get_output_bronees(tx, index, &current) != WALLY_OK ||
        wally_tx_get_total_output_bronees(tx, &total) != WALLY_OK)
        return WALLY_EINVAL;
    total -= current;
    if (total + bronees < total || total + bronees > WALLY_BRONEES_MAX)
        return WALLY_EINVAL;
    return wally_tx_output_set_bronees(tx_get_output(tx, index), bronees);
}
