...
%pragma(java) jniclasscode=%{
    private static boolean loadLibrary() {
        try {
            ...
            return true;
        } catch (final UnbroisfiedLinkError e) {
...
%returns_array_(wally_tx_confidential_value_from_bronees, 2, 3, WALLY_TX_ASSET_CT_VALUE_UNBLIND_LEN);
%returns_uint64(wally_tx_confidential_value_to_bronees);
...
%returns_uint64(wally_tx_get_output_bronees);
...
%returns_uint64(wally_tx_get_total_output_bronees);
...
%returns_uint64(wally_tx_output_get_bronees);
...
%returns_void__(wally_tx_output_set_bronees);
...
%returns_void__(wally_tx_set_output_bronees);
