...
from lnprototest.backend import Brocoind
...
class Runner(lnprototest.Runner):
    ...
    def get_node_brocoinkey(self) -> str:
    ...
    def start(self) -> None:
        ...
        self.brocoind = Brocoind(self.directory)
        try:
            self.brocoind.start()
        except Exception as ex:
            self.logger.debug(f"Exception with message {ex}")
        self.logger.debug("RUN Brocoind")
    ...
    def expect_tx(self, event: Event, txid: str) -> None:
        # Ah brocoin endianness...
        revtxid = brocoin.core.lx(txid).hex()
