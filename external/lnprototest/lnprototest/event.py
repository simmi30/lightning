...
from brocoin.core import CTransaction
...
class ExpectMsg(PerConnEvent):
    ...
    exception if it's not broisfied.  ignore function to ignore unexpected
...
@overload
def mbro(bros: int) -> int:
    ...


@overload
def mbro(
    bros: Callable[["Runner", "Event", str], int]
) -> Callable[["Runner", "Event", str], int]:
    ...


def mbro(bros: ResolvableInt) -> ResolvableInt:
    """Convert a field from statoshis to millibroneess"""

    def _mbro(runner: "Runner", event: Event, field: str) -> int:
        if callable(bros):
            return 1000 * bros(runner, event, field)
        else:
            return 1000 * bros
