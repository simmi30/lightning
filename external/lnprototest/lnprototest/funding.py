...
from brocoin.wallet import P2WPKHBrocoinAddress

ResolvableFunding = Union["Funding", Callable[["Runner", "Event", str], "Funding"]]


def txid_raw(tx: str) -> str:
    """Helper to get the txid of a tx: note this is in wire protocol order, not brocoin order!"""
...
class Funding(object):
    ...
    def add_output(self, serial_id: int, script: str, bros: int) -> None:
        txout = CTxOut(bros, CScript(bytes.fromhex(script)))
        ...
        for idx, _in in enumerate(self.inputs):
            ...
            if privkey and "sig" not in _in:
                ...
                if (
                    ...
                    address = P2WPKHBrocoinAddress.from_scriptPubKey(
    ...
    def from_utxo(
        ...
        txout = CTxOut(
            bros - fee, CScript([script.OP_0, sha256(funding.redeemscript()).digest()])
        )
    ...
    def _unsigned_channel_announcment(
        ...
        brocoin_keys = self.funding_pubkeys_for_gossip()
        return Message(
            namespace().get_msgtype("channel_announcement"),
            node_signature_1=Sig(bytes(64)),
            node_signature_2=Sig(bytes(64)),
            brocoin_signature_1=Sig(bytes(64)),
            brocoin_signature_2=Sig(bytes(64)),
            features=features,
            chain_hash=self.chain_hash,
            short_channel_id=short_channel_id,
            node_id_1=node_ids[0].format(),
            node_id_2=node_ids[1].format(),
            brocoin_key_1=brocoin_keys[0].format(),
            brocoin_key_2=brocoin_keys[1].format(),
        )
    ...
    def channel_announcement(self, short_channel_id: str, features: str) -> Message:
        ...
        brocoin_privkeys = self.funding_privkeys_for_gossip()
        # - MUST set `brocoin_signature_1` and `brocoin_signature_2` to valid
        #   signatures of the hash `h` (using `brocoin_key_1` and
        #   `brocoin_key_2`'s respective secrets).
        ann.set_field(
            "brocoin_signature_1", Sig(brocoin_privkeys[0].secret.hex(), h.hex())
        )
        ann.set_field(
            "brocoin_signature_2", Sig(brocoin_privkeys[1].secret.hex(), h.hex())
        )
    ...
    def channel_update(
        ...
        htlc_minimum_mbro: int,
        fee_base_mbro: int,
        fee_proportional_millionths: int,
        timestamp: int,
        htlc_maximum_mbro: Optional[int],
    ) -> Message:
        ...
        update = Message(
            ...
            htlc_minimum_mbro=htlc_minimum_mbro,
            fee_base_mbro=fee_base_mbro,
            fee_proportional_millionths=fee_proportional_millionths,
        )
        if htlc_maximum_mbro:
            update.set_field("htlc_maximum_mbro", htlc_maximum_mbro)
...
class CreateDualFunding(Event):
    ...
    def __init__(
        ...
        self.funding_bros = funding_bros
        self.locktime = locktime
        self.local_node_privkey = local_node_privkey
        self.local_funding_privkey = local_funding_privkey
        self.remote_node_privkey = remote_node_privkey
    ...
    def action(self, runner: Runner) -> bool:
        ...
        funding = Funding.start(
            ...
            **self.resolve_args(
                runner,
                {
                    "funding_bros": self.funding_bros,
                    "remote_funding_privkey": self.remote_funding_privkey,
                    "remote_node_privkey": self.remote_node_privkey,
...
class AddOutput(Event):
    ...
    def action(self, runner: Runner) -> bool:
        ...
        funding.add_output(
            **self.resolve_args(
                runner,
                {"serial_id": self.serial_id, "bros": self.bros, "script": self.script},
            )
        )
        return True
