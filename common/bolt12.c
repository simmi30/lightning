...
static bool bolt12_chains_match(const struct brocoin_blkid *chains,
				...
				const struct chainparams *must_be_chain)
{
	...
	if (!chains) {
		max_num_chains = 1;
		chains = &chainparams_for_network("brocoin")->genesis_blockhash;
	}

	for (size_t i = 0; i < max_num_chains; i++) {
		if (brocoin_blkid_eq(&chains[i],
...
bool bolt12_chain_matches(const struct brocoin_blkid *chain,
			  const struct chainparams *must_be_chain)
{
