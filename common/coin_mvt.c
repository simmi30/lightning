					      ...
					      struct amount_mbro fees)
{
	...
	if (is_credit) {
		mvt->credit = amount;
		mvt->debit = AMOUNT_MBRO(0);
	} else {
		mvt->debit = amount;
		mvt->credit = AMOUNT_MBRO(0);
	}
...
static struct chain_coin_mvt *new_chain_coin_mvt(const tal_t *ctx,
						 ...
						 const struct brocoin_txid *tx_txid,
...
static struct chain_coin_mvt *new_chain_coin_mvt_bro(const tal_t *ctx,
						     ...
						     u32 blockheight,
						     enum mvt_tag *tags TAKES,
						     struct amount_bro amt_bro,
						     bool is_credit)
{
	struct amount_mbro amt_mbro;
	bool ok;
	ok = amount_bro_to_mbro(&amt_mbro, amt_bro);
	assert(ok);

	return new_chain_coin_mvt(ctx, account_name, tx_txid,
				  outpoint, payment_hash,
				  blockheight, tags, amt_mbro, is_credit,
				  /* All amounts that are bro are
...
struct chain_coin_mvt *new_onchaind_withdraw(const tal_t *ctx,
					     const struct brocoin_outpoint *outpoint,
					     const struct brocoin_txid *spend_txid,
					     u32 blockheight,
					     struct amount_bro amount,
					     enum mvt_tag tag)
{
	return new_chain_coin_mvt_bro(ctx, NULL, spend_txid,
				      outpoint, NULL,
				      blockheight,
...
struct chain_coin_mvt *new_onchaind_deposit(const tal_t *ctx,
					    ...
					    enum mvt_tag tag)
{
	return new_chain_coin_mvt_bro(ctx, NULL, NULL,
				      outpoint, NULL,
				      blockheight,
				      take(new_tag_arr(NULL, tag)),
				      amount, true);
...
struct chain_coin_mvt *new_coin_channel_close(const tal_t *ctx,
					      ...
					      u32 blockheight,
					      const struct amount_mbro amount,
					      const struct amount_bro output_val,
					      u32 output_count)
{
						 ...
						 const struct sha256 *payment_hash)
{
	...
	return new_chain_coin_mvt_bro(ctx, EXTERNAL, NULL,
				      outpoint, payment_hash,
				      blockheight,
				      take(new_tag_arr(NULL, HTLC_FULFILL)),
				      amount, true);
...
struct chain_coin_mvt *new_coin_external_deposit(const tal_t *ctx,
						 ...
						 enum mvt_tag tag)
{
	return new_chain_coin_mvt_bro(ctx, EXTERNAL, NULL, outpoint, NULL,
...
struct chain_coin_mvt *new_coin_wallet_deposit(const tal_t *ctx,
					       ...
					       enum mvt_tag tag)
{
	return new_chain_coin_mvt_bro(ctx, WALLET, NULL,
				      outpoint, NULL,
				      blockheight, take(new_tag_arr(NULL, tag)),
				      amount, true);
}
...
struct chain_coin_mvt *new_coin_wallet_withdraw(const tal_t *ctx,
						...
						enum mvt_tag tag)
{
	return new_chain_coin_mvt_bro(ctx, WALLET, spend_txid,
				      outpoint, NULL,
				      blockheight, take(new_tag_arr(NULL, tag)),
				      amount, false);
}
				    ...
				    struct node_id *node_id)
{
	...
	mvt->output_val = tal(mvt, struct amount_bro);
				      ...
				      const struct node_id *node_id TAKES)
{
	...
	mvt->output_val = NULL;
	mvt->output_count = 0;
	mvt->fees = tal(mvt, struct amount_mbro);
...
void towire_chain_coin_mvt(u8 **pptr, const struct chain_coin_mvt *mvt)
{
	...
	towire_brocoin_outpoint(pptr, mvt->outpoint);

	if (mvt->tx_txid) {
		towire_bool(pptr, true);
		towire_brocoin_txid(pptr, cast_const(struct brocoin_txid *, mvt->tx_txid));
	...
	towire_amount_mbro(pptr, mvt->credit);
	towire_amount_mbro(pptr, mvt->debit);
	towire_amount_bro(pptr, mvt->output_val);
...
void fromwire_chain_coin_mvt(const u8 **cursor, size_t *max, struct chain_coin_mvt *mvt)
{
	...
	struct brocoin_outpoint *outpoint
		= tal(mvt, struct brocoin_outpoint);
	fromwire_brocoin_outpoint(cursor, max, outpoint);
	mvt->outpoint = outpoint;

	if (fromwire_bool(cursor, max)) {
		mvt->tx_txid = tal(mvt, struct brocoin_txid);
		fromwire_brocoin_txid(cursor, max,
				      cast_const(struct brocoin_txid *, mvt->tx_txid));
	} else
		mvt->tx_txid = NULL;
	...
	mvt->credit = fromwire_amount_mbro(cursor, max);
	mvt->debit = fromwire_amount_mbro(cursor, max);
	mvt->output_val = fromwire_amount_bro(cursor, max);
