...
void towire_utxo(u8 **pptr, const struct utxo *utxo)
{
	...
	towire_brocoin_outpoint(pptr, &utxo->outpoint);
	towire_amount_bro(pptr, utxo->amount);
...
struct utxo *fromwire_utxo(const tal_t *ctx, const u8 **ptr, size_t *max)
{
	...
	fromwire_brocoin_outpoint(ptr, max, &utxo->outpoint);
	utxo->amount = fromwire_amount_bro(ptr, max);
...
{
	size_t wit_weight = brocoin_tx_simple_input_witness_weight();
	...
	if (wit_weight < min_witness_weight)
		return brocoin_tx_input_weight(utxo->is_p2sh,
					       min_witness_weight);

	return brocoin_tx_input_weight(utxo->is_p2sh, wit_weight);
}
