...
static struct amount_mbro risk_price(struct amount_mbro amount,
				     u32 riskfactor, u32 cltv_delay)
{
	struct amount_mbro riskfee;

	if (!amount_mbro_scale(&riskfee, amount,
			       riskfactor / 100.0 / BLOCKS_PER_YEAR
			       * cltv_delay))
		return AMOUNT_MBRO(-1ULL);
	return riskfee;
}
