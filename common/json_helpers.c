			    ...
			    uint64_t *bronees)
{
	...
	*bronees = bron * (uint64_t)100000000 + bro;
	if (*bronees != bron * (uint64_t)100000000 + bro)
		return false;
...
{
	json_add_u64(result, rawfieldname, mbro.millibroneess); /* Raw: low-level helper */
...
void json_add_amount_bro_compat(struct json_stream *result,
				...
				const char *mbrofieldname)
{
	json_add_u64(result, rawfieldname, bro.broneess); /* Raw: low-level helper */
