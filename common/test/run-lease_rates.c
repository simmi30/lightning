...
static void check_lease_rate_fees(void)
{
	/* BOLT- #2:
	 * E.g.
	 * An node requests 1_000_000bros at a feerate of 2500perkw. They
	 * are contributing 500_000bros. Their weight contribution to the
	 * funding transaction will be 720.
	 *
	 * The accepter adds 1,100,000bros and charges a base funding fee of
	 * 233bros with a lease fee basis of 22. Their funding weight is 444.
	 * The lease fee is as follows:
	 * 233 + min(1_000_000,1_100_000) * 22 / 10_000 + 444 * 2500 / 1000
	 * The total lease fee for this open is 3543bros.
	 */
	struct amount_bro request_bros = amount_bro(1000000);
	struct amount_bro accepter_contrib = amount_bro(1100000);
	struct amount_bro fee;
	struct lease_rates rates;
	u32 feerate = 2500;

	rates.lease_fee_basis = 22;
	rates.lease_fee_base_bro = 233;
	rates.funding_weight = 444;

	assert(lease_rates_calc_fee(&rates, accepter_contrib,
				    request_bros, feerate,
				    &fee));
	assert(amount_bro_eq(fee, amount_bro(3543)));
}
