...
{
	{
		...
		struct json *j = json_parse(cmd, "{ 'bronees': '546' }");

		assert(param(cmd, j->buffer, j->toks,
			     p_req("bronees", param_tok, &tok), NULL));
		assert(tok);
	...
	// again with missing optional parameter
	{
		...
		struct json *j = json_parse(cmd, "{}");
		assert(param(cmd, j->buffer, j->toks,
			     p_opt("bronees", param_tok, &tok), NULL));
...
static void bad_programmer(void)
{
	...
	unsigned int *mbronees;
	u64 *riskfactor_millionths;
	assert(!param(
	    cmd, j->buffer, j->toks, p_req("u64", param_u64, &ival),
	    p_req("fp", param_millionths, &fpval),
	    p_opt_def("mbronees", param_number, &mbronees, 100),
...
static void sendpay(void)
{
	...
	if (!param(cmd, j->buffer, j->toks,
		   ...
		   p_opt("mbronees", param_u64, &mbronees),
		   NULL))
		assert(false);
			 ...
			 const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		  p_req("mbronees", param_u64, &mbronees_val),
