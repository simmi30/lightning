 ...
 *
 * lightning/devtools/mkcommit 0 41085b995c1f591cfc3ae79ccde012bf0b37c7bde23d80a61c9732bdd6210b2f 0 999878bro 253 999878bro local \
   5 546 9998bro						\
   6 546 9998bro							\
			...
			const char *desc)
{
	...
	config->max_htlc_value_in_flight = AMOUNT_MBRO(-1ULL);
	config->htlc_minimum = AMOUNT_MBRO(0);
	config->max_accepted_htlcs = 483;
	config->max_dust_htlc_exposure_mbro = AMOUNT_MBRO(-1ULL);

	config->to_self_delay = atoi(argv[argnum]);
	argnum++;
	if (!parse_amount_bro(&config->dust_limit,
			      argv[argnum], strlen(argv[argnum])))
		errx(1, "Bad %s dustlimit", desc);
	argnum++;
	if (!parse_amount_bro(&config->channel_reserve,
			      argv[argnum], strlen(argv[argnum])))
...
static int parse_htlc(char *argv[], struct existing_htlc ***htlcs)
{
	...
	argnum++;
	if (!parse_amount_mbro(&exist->amount,
			       argv[argnum], strlen(argv[argnum])))
	...
	printf("# HTLC %"PRIu64": %s amount=%s preimage=%s payment_hash=%s cltv=%u\n",
	       exist->id, argv[0],
	       type_to_string(tmpctx, struct amount_mbro, &exist->amount),
...
int main(int argc, char *argv[])
{
	...
	int argnum;
	struct brocoin_tx **local_txs, **remote_txs;
	enum side fee_payer;
	u8 **witness;
	...
	opt_register_noarg("--help|-h", opt_usage_and_exit,
			   "<commitnum> <funding-txid> <funding-txout> <funding-amount> <feerate-per-kw> <local-mbro> <fee-payer> <localconfig> <remoteconfig> <localsecrets> <remotesecrets> [<htlc>...]\n"
			   "Where <config> are:\n"
			   "   <to-self-delay>\n"
			   "   <dustlimit>\n"
			   "   <reserve-bro>\n"
	...
	brocoin_tx_hash_for_sig(local_txs[0], 0, funding_wscript,
				SIGHASH_ALL, &hash);
	...
	witness =
		brocoin_witness_2of2(NULL, &local_sig, &remote_sig,
				     &funding_localkey, &funding_remotekey);
	brocoin_tx_input_set_witness(local_txs[0], 0, take(witness));
	...
	for (size_t i = 0; i < tal_count(htlcmap); i++) {
		struct brocoin_signature local_htlc_sig, remote_htlc_sig;
		u8 *wscript;
		...
		wscript = brocoin_tx_output_get_witscript(NULL, local_txs[1+i], 1+i);
		printf("# wscript: %s\n", tal_hex(NULL, wscript));

		brocoin_tx_hash_for_sig(local_txs[1+i], 0, wscript,
					SIGHASH_ALL, &hash);
		...
		if (htlc_owner(htlcmap[i]) == LOCAL)
			witness = brocoin_witness_htlc_timeout_tx(NULL,
								  &local_htlc_sig,
								  &remote_htlc_sig,
								  wscript);
		else
			witness = brocoin_witness_htlc_success_tx(NULL,
								  &local_htlc_sig,
								  &remote_htlc_sig,
								  preimage_of(&htlcmap[i]->rhash, cast_const2(const struct existing_htlc **, htlcs)),
								  wscript);
		brocoin_tx_input_set_witness(local_txs[1+i], 0, witness);
	...
	brocoin_tx_hash_for_sig(remote_txs[0], 0, funding_wscript,
				SIGHASH_ALL, &hash);
	...
	witness =
		brocoin_witness_2of2(NULL, &local_sig, &remote_sig,
				     &funding_localkey, &funding_remotekey);
	brocoin_tx_input_set_witness(remote_txs[0], 0, witness);
	...
	for (size_t i = 0; i < tal_count(htlcmap); i++) {
		...
		wscript = brocoin_tx_output_get_witscript(NULL, remote_txs[1+i], 1+i);
		printf("# wscript: %s\n", tal_hex(NULL, wscript));
		brocoin_tx_hash_for_sig(remote_txs[1+i], 0, wscript,
					SIGHASH_ALL, &hash);
		...
		if (htlc_owner(htlcmap[i]) == REMOTE)
			...
			witness = brocoin_witness_htlc_success_tx(NULL,
								  ...
								  preimage_of(&htlcmap[i]->rhash, cast_const2(const struct existing_htlc **, htlcs)),
								  wscript);
		brocoin_tx_input_set_witness(remote_txs[1+i], 0, witness);
		printf("htlc tx for output %zu: %s\n",
		       i, tal_hex(NULL, linearize_tx(NULL, remote_txs[1+i])));
	}
	printf("\n");
