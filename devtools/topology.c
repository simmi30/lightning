				     ...
				     bool is_last_node)
{
	...
	dij = dijkstra(tmpctx, map, n, AMOUNT_MBRO(0), 0,
		  ...
		  bool *visited)
{
	...
	for (size_t i = 0; i < n->num_chans; i++) {
		...
		if (!channel_usable(map, c, dir, AMOUNT_MBRO(0), NULL))
			continue;
					  ...
					  bool is_first_node)
{
	...
	dij = dijkstra(tmpctx, map, start, AMOUNT_MBRO(0), 0,
			       ...
			       struct gossmap_node *dst)
{
	...
	/* 10kbro, budget is 0.5% */
	const struct amount_mbro sent = AMOUNT_MBRO(10000000);
	const struct amount_mbro budget = amount_mbro_div(sent, 200);
	const u32 riskfactor = 0;
	/* Max distance is 20 */
	const u32 distance_budget = ROUTING_MAX_HOPS;
	struct amount_mbro maxcost, fee;
	...
	if (!amount_mbro_add(&maxcost, sent, budget))
		abort();

	path = route_from_dijkstra(map, map, dij, src, sent, 0);

	if (amount_mbro_greater(path[0].amount, maxcost)) {
		printf("failed (too expensive)\n");
		return false;
	}
	printf("# path length %zu\n", tal_count(path));
	if (!amount_mbro_sub(&fee, path[0].amount, sent))
		abort();
	printf("# path fee %s\n",
