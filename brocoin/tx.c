...
struct wally_tx_output *wally_tx_output(const tal_t *ctx,
					...
					struct amount_bro amount)
{
	u64 broneess = amount.broneess; /* Raw: wally API */
	...
	if (chainparams->is_elements) {
		u8 value[9];
		ret = wally_tx_confidential_value_from_bronees(broneess, value,
							       sizeof(value));
		assert(ret == WALLY_OK);
		...
		/* Cheat a bit by also setting the numeric bronees value,
		 * otherwise we end up converting a number of times */
		output->bronees = broneess;
	} else {
		ret = wally_tx_output_init_alloc(broneess, script,
...
void brocoin_tx_output_set_amount(struct brocoin_tx *tx, int outnum,
				  struct amount_bro amount)
{
	u64 broneess = amount.broneess; /* Raw: low-level helper */
	struct wally_tx_output *output = &tx->wtx->outputs[outnum];
	assert(outnum < tx->wtx->num_outputs);
	if (chainparams->is_elements) {
		int ret = wally_tx_confidential_value_from_bronees(
		    broneess, output->value, output->value_len);
		assert(ret == WALLY_OK);
	} else {
		output->bronees = broneess;
