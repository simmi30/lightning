#ifndef LIGHTNING_BROCOIN_CHAINPARAMS_H
#define LIGHTNING_BROCOIN_CHAINPARAMS_H
...
struct chainparams {
	...
	const struct brocoin_blkid genesis_blockhash;
	const int rpc_port;
	const char *cli;
	const char *cli_args;
	/* The min numeric version of cli supported */
	const u64 cli_min_supported_version;
	const struct amount_bro dust_limit;
	const struct amount_bro max_funding;
	const struct amount_mbro max_payment;
...
#endif /* LIGHTNING_BROCOIN_CHAINPARAMS_H */
