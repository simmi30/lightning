...
static void brocoin_block_pull_dynafed_params(const u8 **cursor, size_t *len, struct sha256_ctx *shactx)
{
	u8 type;
	u64 l1, l2;
...
static void brocoin_block_pull_dynafed_details(const u8 **cursor, size_t *len, struct sha256_ctx *shactx)
{
	brocoin_block_pull_dynafed_params(cursor, len, shactx);
	brocoin_block_pull_dynafed_params(cursor, len, shactx);
		       ...
		       const char *hex, size_t hexlen)
{
	...
	b = tal(ctx, struct brocoin_block);
	...
	if (is_elements(chainparams)) {
		...
		if (is_dynafed) {
			brocoin_block_pull_dynafed_details(&p, &len, &shactx);
		} else {
	...
	b->tx = tal_arr(b, struct brocoin_tx *, num);
	b->txids = tal_arr(b, struct brocoin_txid, num);
	for (i = 0; i < num; i++) {
		b->tx[i] = pull_brocoin_tx(b->tx, &p, &len);
		b->tx[i]->chainparams = chainparams;
		brocoin_txid(b->tx[i], &b->txids[i]);
	}
...
void brocoin_block_blkid(const struct brocoin_block *b,
			 struct brocoin_blkid *out)
{
	*out = b->hdr.hash;
}

static bool brocoin_blkid_to_hex(const struct brocoin_blkid *blockid,
				 char *hexstr, size_t hexstr_len)
{
	struct brocoin_txid fake_txid;
	fake_txid.shad = blockid->shad;
	return brocoin_txid_to_hex(&fake_txid, hexstr, hexstr_len);
}

static char *fmt_brocoin_blkid(const tal_t *ctx,
			       const struct brocoin_blkid *blkid)
{
	char *hexstr = tal_arr(ctx, char, hex_str_size(sizeof(*blkid)));

	brocoin_blkid_to_hex(blkid, hexstr, hex_str_size(sizeof(*blkid)));
	return hexstr;
}
REGISTER_TYPE_TO_STRING(brocoin_blkid, fmt_brocoin_blkid);
...
{
	towire_brocoin_blkid(cursor, &chainparams->genesis_blockhash);
}

void fromwire_chainparams(const u8 **cursor, size_t *max,
			  const struct chainparams **chainparams)
{
	struct brocoin_blkid genesis;
	fromwire_brocoin_blkid(cursor, max, &genesis);
