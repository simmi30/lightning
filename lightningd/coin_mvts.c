						     ...
						     struct channel *channel)
{
	return new_channel_coin_mvt(ctx, &channel->cid,
				    ...
				    hin->mbro, new_tag_arr(ctx, INVOICE),
				    true, AMOUNT_MBRO(0));
}
...
struct channel_coin_mvt *new_channel_mvt_routed_hin(const tal_t *ctx,
						    ...
						    struct channel *channel)
{
	struct amount_mbro fees_collected;

	if (!hin->payload)
		return NULL;

	if (!amount_mbro_sub(&fees_collected, hin->mbro,
			     hin->payload->amt_to_forward))
		return NULL;

	return new_channel_coin_mvt(ctx, &channel->cid,
				    &hin->payment_hash, NULL,
				    hin->mbro, new_tag_arr(ctx, ROUTED),
				    true, fees_collected);
}
						      ...
						      struct channel *channel)
{
	return new_channel_coin_mvt(ctx, &channel->cid,
				    ...
				    hout->mbro, new_tag_arr(ctx, INVOICE),
				    false, hout->fees);
}
						     ...
						     struct channel *channel)
{
	return new_channel_coin_mvt(ctx, &channel->cid,
				    ...
				    hout->mbro, new_tag_arr(ctx, ROUTED),
				    false,
				    hout->fees);
}
	...
	for (size_t i = 0; i < ARRAY_SIZE(utxo_states); i++) {
		...
		for (size_t j = 0; j < tal_count(utxos); j++) {
			...
			if (!utxos[j]->spendheight && !utxos[j]->blockheight)
				continue;
			if (!amount_mbro_add_bro(&bal->balance,
