...
void notify_invoice_payment(struct lightningd *ld, struct amount_mbro amount,
...
void notify_invoice_creation(struct lightningd *ld, struct amount_mbro *amount,
			     struct preimage preimage,
			     const struct json_escape *label)
{
	void (*serialize)(struct json_stream *,
						  ...
						  bool *funding_locked)
{
	...
	json_add_amount_bro_only(stream, "amount", *funding_bro);
...
void notify_channel_opened(struct lightningd *ld, struct node_id *node_id,
			   struct amount_bro *funding_bro, struct brocoin_txid *funding_txid,
			   bool *funding_locked)
{
	void (*serialize)(struct json_stream *,
			  struct node_id *,
	...
	serialize(n->stream, node_id, funding_bro, funding_txid, funding_locked);
	jsonrpc_notification_end(n);
	plugins_notify(ld->plugins, take(n));
}
...
static void forward_event_notification_serialize(struct json_stream *stream,
						 ...
						 const struct amount_mbro *amount_out,
						 ...
						 enum forward_style forward_style)
{
	...
	if (scid_out) {
		...
		if (amount_out) {
			cur->mbro_out = *amount_out;
			if (!amount_mbro_sub(&cur->fee,
					     in->mbro, *amount_out))
				abort();
		} else {
						 ...
						 struct coin_mvt *mvt)
{
	...
	json_add_amount_mbro_only(stream, "credit", mvt->credit);
	json_add_amount_mbro_only(stream, "debit", mvt->debit);
	/* Only chain movements */
	if (mvt->output_val)
		json_add_amount_bro_only(stream, "output_value",
					 *mvt->output_val);
	...
	if (mvt->fees)
		json_add_amount_mbro_only(stream, "fees",
					  *mvt->fees);
...
static void balance_snapshot_notification_serialize(struct json_stream *stream, struct balance_snapshot *snap)
{
	...
	for (size_t i = 0; i < tal_count(snap->accts); i++) {
		...
		json_add_amount_mbro_only(stream, "balance",
