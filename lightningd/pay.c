			     ...
			     const struct wallet_payment *t)
{
	...
	if (amount_mbro_greater(t->mbronees, AMOUNT_MBRO(0)))
		json_add_amount_mbro_compat(response, t->mbronees, "mbronees",
					    "amount_mbro");

	json_add_amount_mbro_compat(response, t->mbronees_sent,
				    "mbronees_sent", "amount_sent_mbro");
		  ...
		  const struct sha256 *local_offer_id)
{
	...
	for (size_t i = 0; i < tal_count(payments); i++) {
		...
		case PAYMENT_COMPLETE:
			...
			if (!amount_mbro_eq(payments[i]->mbronees, mbro)) {
		...
		case PAYMENT_PENDING:
			 ...
			 * sent, since we'll use it in onion! */
				...
				return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
						    "mbronees was previously %s, now %s",
						    type_to_string(tmpctx,
								   struct amount_mbro,
	...
	payment->mbronees = mbro;
	payment->mbronees_sent = first_hop->amount;
					     ...
					     const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt_def("mbronees", param_mbro, &mbro, AMOUNT_MBRO(0)),
	...
	*hops = tal_arr(cmd, struct route_hop, tok->size);
	json_for_each_arr(i, t, tok) {
		...
		if (!param(cmd, buffer, t,
			   ...
			   p_opt("mbronees", param_mbro, &mbro),
					   ...
					   const jsmntok_t *params)
{
	...
	if (*partid && !mbro)
		return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
				    "Must specify mbronees with partid");
	...
	if (mbro && !*partid && !amount_mbro_eq(*mbro, final_amount))
		return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
				    "Do not specify mbronees (%s) without"
	 ...
	 * we promise to send (mbronees).  So no single payment can be
	 * > than that. */
	if (*partid) {
