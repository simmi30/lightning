...
static char *opt_set_mbro(const char *arg, struct amount_mbro *amt)
{
	if (!parse_amount_mbro(amt, arg, strlen(arg)))
		return tal_fmt(NULL, "Unable to parse millibronees '%s'", arg);
...
static void register_opts(struct lightningd *ld)
{
	...
	opt_register_arg("--fee-base", opt_set_u32, opt_show_u32,
			 &ld->config.fee_base,
			 "Millibronees minimum to charge for HTLC");
	...
	opt_register_arg("--fee-per-bronees", opt_set_u32, opt_show_u32,
			 &ld->config.fee_per_bronees,
			 "Microbronees fee for every bronees in HTLC");
	...
	opt_register_arg("--min-capacity-bro", opt_set_u64, opt_show_u64,
			 ...
			 "Minimum capacity in broneess for accepting channels");
