		      ...
		      const struct channel_type *type)
{
	...
	channel = new_channel(uc->peer, uc->dbid,
			      ...
			      NULL, /* no scid yet */
			      cid,
			      /* The three arguments below are mbronees_to_us,
			       * mbronees_to_us_min, and mbronees_to_us_max.
...
struct openchannel_hook_payload {
	...
	struct uncommitted_channel* uc;
	struct amount_bro funding_broneess;
	struct amount_mbro push_mbro;
	struct amount_bro dust_limit_broneess;
	struct amount_mbro max_htlc_value_in_flight_mbro;
	struct amount_bro channel_reserve_broneess;
				       ...
				       struct plugin *plugin)
{
	...
	json_add_amount_bro_only(stream, "funding_broneess",
				 payload->funding_broneess);
	json_add_amount_mbro_only(stream, "push_mbro", payload->push_mbro);
	json_add_amount_bro_only(stream, "dust_limit_broneess",
				 payload->dust_limit_broneess);
	json_add_amount_mbro_only(stream, "max_htlc_value_in_flight_mbro",
				  payload->max_htlc_value_in_flight_mbro);
	json_add_amount_bro_only(stream, "channel_reserve_broneess",
				 payload->channel_reserve_broneess);
...
static const struct json_command fundchannel_start_command = {
    ...
    "Start fund channel with {id} using {amount} broneess. "
