 ...
 * between them, handle the JSON RPC requests, brocoind, the database
...
static struct lightningd *new_lightningd(const tal_t *ctx)
{
	...
	ld->dev_max_funding_unconfirmed = 10080;
...
int main(int argc, char *argv[])
{
	 ...
	 * similarly to what Brocoin Core does to ports by default. */
	 ...
	 * brocoin wallet (though it's that too).  It also stores channel
	 * states, invoices, payments, blocks and brocoin transactions. */
	 ...
	 * an uninitialized wallet and that we should start off of brocoind's
	 * current height */
	 ...
	 * talk to brocoind, so does its own db transactions. */
	 ...
	 *  can start the poll loop which queries brocoind for new blocks. */
