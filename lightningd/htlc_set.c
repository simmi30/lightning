				     ...
				     struct amount_mbro total_mbro)
{
	...
	set->total_mbro = total_mbro;
	set->payment_hash = hin->payment_hash;
	set->so_far = AMOUNT_MBRO(0);
		  ...
		  const struct secret *payment_secret)
{
	...
	if (!set)
		set = new_htlc_set(ld, hin, total_mbro);
	else {
		/* BOLT #4:
		 *
		 * if it supports `basic_mpp`:
		 * ...
		 *  - otherwise, if the total `amount_mbro` of this HTLC set is
		 *    less than `total_mbro`:
		 * ...
	...
	/* BOLT #4:
	 * - SHOULD fail the entire HTLC set if `total_mbro` is not
	 *   the same for all HTLCs in the set.
	 */
	if (!amount_mbro_eq(total_mbro, set->total_mbro)) {
		log_unusual(ld->log, "Failing HTLC set %s:"
			    " total_mbro %s new htlc total %s",
			    type_to_string(tmpctx, struct sha256,
					   &set->payment_hash),
	 ...
	 */
	if (!amount_mbro_add(&set->so_far, set->so_far, hin->mbro)) {
		log_unusual(ld->log, "Failing HTLC set %s:"
			    " overflow adding %s+%s",
			    type_to_string(tmpctx, struct sha256,
					   &set->payment_hash),
	...
	log_debug(ld->log,
		  ...
		  tal_count(set->htlcs),
		  type_to_string(tmpctx, struct amount_mbro, &set->so_far),
		  type_to_string(tmpctx, struct amount_mbro, &total_mbro),
		  payment_secret ? "" : "no "
		);

	if (amount_mbro_eq(set->so_far, total_mbro)) {
