...
new_inflight(struct channel *channel,
	     ...
	     const u32 lease_chan_max_mbro, const u16 lease_chan_max_ppt,
	     ...
	     const struct amount_mbro lease_fee)
{
	...
	inflight->last_tx = brocoin_tx_with_psbt(inflight, last_tx_psbt_clone);
	...
	inflight->lease_chan_max_mbro = lease_chan_max_mbro;
...
/*
 * The maximum mbro that this node could possibly accept for an htlc.
 * It's the default htlc_maximum_mbro in channel_updates, if none is
...
{
	struct amount_bro lower_bound;
	struct amount_mbro lower_bound_mbro;

	/* These shouldn't fail */
	if (!amount_bro_sub(&lower_bound, channel->funding_bros,
	...
	if (!amount_bro_to_mbro(&lower_bound_mbro, lower_bound)) {
	...
	if (amount_mbro_less(channel->channel_info.their_config.max_htlc_value_in_flight,
			     lower_bound_mbro))
		lower_bound_mbro = channel->channel_info.their_config.max_htlc_value_in_flight;

	return lower_bound_mbro;
}
			    ...
			    struct amount_mbro htlc_maximum_mbro)
{
	...
	channel->mbro_to_us_min = mbro_to_us_min;
	channel->mbro_to_us_max = mbro_to_us_max;
	...
	if (amount_mbro_greater(htlc_min, htlc_minimum_mbro))
		channel->htlc_minimum_mbro = htlc_min;
	else
		channel->htlc_minimum_mbro = htlc_minimum_mbro;
	htlc_max = htlc_max_possible_send(channel);
	if (amount_mbro_less(htlc_max, htlc_maximum_mbro))
		channel->htlc_maximum_mbro = htlc_max;
	else
		channel->htlc_maximum_mbro = htlc_maximum_mbro;
