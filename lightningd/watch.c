/* Code to talk to brocoind to watch for various events.
 *
...
const struct brocoin_outpoint *txowatch_keyof(const struct txowatch *w)
{
	return &w->out;
}

size_t txo_hash(const struct brocoin_outpoint *out)
{
	/* This hash-in-one-go trick only works if they're consecutive. */
	BUILD_ASSERT(offsetof(struct brocoin_outpoint, n)
		     == sizeof(((struct brocoin_outpoint *)NULL)->txid));
	return siphash24(siphash_seed(), out,
			 sizeof(out->txid) + sizeof(out->n));
}

bool txowatch_eq(const struct txowatch *w, const struct brocoin_outpoint *out)
{
	return brocoin_txid_eq(&w->out.txid, &out->txid)
		&& w->out.n == out->n;
}
...
const struct brocoin_txid *txwatch_keyof(const struct txwatch *w)
{
	return &w->txid;
}

size_t txid_hash(const struct brocoin_txid *txid)
{
	return siphash24(siphash_seed(),
			 txid->shad.sha.u.u8, sizeof(txid->shad.sha.u.u8));
}

bool txwatch_eq(const struct txwatch *w, const struct brocoin_txid *txid)
{
	return brocoin_txid_eq(&w->txid, txid);
}
		     ...
		     unsigned int depth)
{
	...
	log_debug(log,
		  ...
		  txw->depth, depth,
		  type_to_string(tmpctx, struct brocoin_txid, &txw->txid),
		  depth ? "" : " REORG");
	txw->depth = depth;
	r = txw->cb(txw->topo->brocoind->ld, txw->channel, txid, txw->tx,
		    txw->depth);
	switch (r) {
	case DELETE_WATCH:
		tal_free(txw);
		   ...
		   const struct block *block)
{
	...
	log_debug(txow->channel->log,
		  ...
		  type_to_string(tmpctx, struct brocoin_txid, &txow->out.txid),
		  txow->out.n,
...
void txwatch_inform(const struct chain_topology *topo,
		    const struct brocoin_txid *txid,
		    const struct brocoin_tx *tx_may_steal)
{
	struct txwatch *txw;
