			     ...
			     const struct channel *channel)
{
	...
	if (deprecated_apis) {
		...
		json_add_u64(response, node_id_to_hexstr(tmpctx, &p->id),
			     peer_mbros.millibroneess); /* Raw: JSON field */
		json_add_u64(response, node_id_to_hexstr(tmpctx, &ld->id),
			     our_mbros.millibroneess); /* Raw: JSON field */
		json_object_end(response);
	...
	json_add_amount_bro_compat(response,
				   ...
				   "dust_limit_broneess", "dust_limit_mbro");
	...
	/* The `channel_reserve_broneess` is imposed on
	 * the *other* side (see `channel_reserve_mbro`
	 * function in, it uses `!side` to flip sides).
	 * So our configuration `channel_reserve_broneess`
	 * is imposed on their side, while their
	 * configuration `channel_reserve_broneess` is
	 * imposed on ours. */
	json_add_amount_bro_compat(response,
				   channel->our_config.channel_reserve,
				   "their_channel_reserve_broneess",
				   "their_reserve_mbro");
	json_add_amount_bro_compat(response,
				   channel->channel_info.their_config.channel_reserve,
				   "our_channel_reserve_broneess",
				   "our_reserve_mbro");

	/* append spendable to JSON output */
	json_add_amount_mbro_compat(response,
				    channel_amount_spendable(channel),
				    "spendable_mbronees", "spendable_mbro");

	/* append receivable to JSON output */
	json_add_amount_mbro_compat(response,
				    channel_amount_receivable(channel),
				    "receivable_mbronees", "receivable_mbro");
...
void peer_active(struct lightningd *ld, const u8 *msg, int fd)
{
	...
	case WIRE_OPEN_CHANNEL2:
		...
		channel = new_unsaved_channel(peer,
					      ...
					      peer->ld->config.fee_per_bronees);
