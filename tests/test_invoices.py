...
def test_invoice_preimage(node_factory):
    ...
    inv = l2.rpc.invoice(mbronees=123456, label="inv", description="?", preimage=invoice_preimage)
...
def test_invoice_routeboost(node_factory, brocoind):
    ...
    # Check routeboost.
    # Make invoice and pay it
    inv = l2.rpc.invoice(mbronees=123456, label="inv1", description="?")
    # Check routeboost.
    ...
    inv = l2.rpc.invoice(mbronees=2 * (10**8) - 123456, label="inv2", description="?")
    # Check warning
    assert 'warning_capacity' in inv
    assert 'warning_private_unused' not in inv
    assert 'warning_offline' not in inv
...
def test_invoice_routeboost_private(node_factory, brocoind):
    ...
    # though it's private
    inv = l2.rpc.invoice(mbronees=123456, label="inv0", description="?")
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=123456, label="inv1", description="?", exposeprivatechannels=False)
    ...
    inv = l2.rpc.invoice(mbronees=123456, label="inv1a", description="?", exposeprivatechannels=scid)
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=123456, label="inv1b", description="?", exposeprivatechannels=[scid])
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=10**7, label="inv2", description="?")
    print(inv)
    assert 'warning_deadends' in inv
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=10**7, label="inv3", description="?", exposeprivatechannels=True)
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=10**7, label="inv4", description="?", exposeprivatechannels=scid)
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=(10**5) * 1000 + 1, label="inv5", description="?", exposeprivatechannels=scid2)
    assert 'warning_private_unused' not in inv
    assert 'warning_deadends' not in inv
    assert 'warning_capacity' in inv
    assert 'warning_offline' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=(10**5) * 1000 + 1, label="inv6", description="?", exposeprivatechannels=[scid2, scid])
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
    ...
    inv = l2.rpc.invoice(mbronees=123456, label="inv7", description="?", exposeprivatechannels=scid)
    assert 'warning_private_unused' not in inv
    assert 'warning_capacity' not in inv
    assert 'warning_offline' not in inv
    assert 'warning_deadends' not in inv
...
def test_invoice_expiry(node_factory, executor):
    ...
    inv = l2.rpc.invoice(mbronees=123000, label='test_pay', description='description', expiry=1)['bolt11']
    time.sleep(2)
    ...
    start = int(time.time())
    inv = l2.rpc.invoice(mbronees=123000, label='inv_s', description='description', expiry=1)['bolt11']
    end = int(time.time())
...
def test_autocleaninvoice(node_factory):
    ...
    l1.rpc.invoice(mbronees=12300, label='inv1', description='description1', expiry=4)
    l1.rpc.invoice(mbronees=12300, label='inv2', description='description2', expiry=12)
...
def test_amountless_invoice(node_factory):
    ...
    assert(len(i) == 1)
    assert('mbronees_received' not in i[0])
    assert('amount_received_mbro' not in i[0])
    assert(i[0]['status'] == 'unpaid')
    details = l1.rpc.decodepay(inv)
    assert('mbronees' not in details)

    l1.rpc.pay(inv, mbronees=1337)

    i = l2.rpc.listinvoices()['invoices']
    assert(len(i) == 1)
    assert(i[0]['mbronees_received'] == 1337)
    assert(i[0]['amount_received_mbro'] == Millibronees(1337))
