...
We just refuse to let them open channels with an odd amount of millibroneess.
"""
...
def run_check(funding_amt_str):
    if Millibronees(funding_amt_str).to_bronees() % 2 == 1:
...
def on_openchannel(openchannel, plugin, **kwargs):
    ...
    return run_check(openchannel['funding_broneess'])
