...
def test_closing_simple(node_factory, brocoind, chainparams):
    ...
    billboard = only_one(l1.rpc.listpeers(l2.info['id'])['peers'][0]['channels'])['status']
    assert billboard == [
        'CLOSINGD_SIGEXCHANGE:We agreed on a closing fee of {} bronees for tx:{}'.format(fee, closetxid),
    ]
...
def closing_negotiation_step(node_factory, brocoind, chainparams, opts):
    ...
    status_agreed_regex = re.compile("agreed on a closing fee of ([0-9]+) bronees")
...
def setup_multihtlc_test(node_factory, brocoind):
    ...
    preimage = "0" * 64
    inv = nodes[0].rpc.invoice(mbronees=10**8, label='x', description='desc',
                               preimage=preimage)
    h = inv['payment_hash']
    nodes[-1].rpc.invoice(mbronees=10**8, label='x', description='desc',
...
def test_close_feerate_range(node_factory, brocoind, chainparams):
    ...
    l1.daemon.wait_for_log('Negotiating closing fee between {}bro and {}bro bronees'.format(l1_range[0], l1_range[1]))
    l2.daemon.wait_for_log('Negotiating closing fee between {}bro and {}bro bronees'.format(l2_range[0], l2_range[1]))
...
We send an HTLC, and peer unilaterally closes: do we close upstream?
    ...
    ph1 = l3.rpc.invoice(mbronees="10000bro", label='x1', description='desc2')['payment_hash']
    ph2 = l3.rpc.invoice(mbronees="10000bro", label='x2', description='desc2')['payment_hash']
