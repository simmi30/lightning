...
def test_gossip_lease_rates(node_factory, brocoind):
    ...
    assert rates['channel_fee_max_base_mbro'] == Millibronees('500000mbro')
    assert rates['channel_fee_max_proportional_thousandths'] == 200
    assert rates['funding_weight'] == 666  # Default on regtest
    assert rates['lease_fee_base_mbro'] == Millibronees('2000mbro')
    ...
    assert rates['channel_fee_max_base_mbro'] == Millibronees('30000mbro')
    assert rates['channel_fee_max_proportional_thousandths'] == 100
    assert rates['funding_weight'] == 666  # Default on regtest
    assert rates['lease_fee_base_mbro'] == Millibronees('400000mbro')
...
def test_node_reannounce(node_factory, brocoind, chainparams):
    ...
    assert ad['lease_fee_base_mbro'] == Millibronees('1000000mbro')
    assert ad['lease_fee_basis'] == 20
    assert ad['funding_weight'] == 150
    assert ad['channel_fee_max_base_mbro'] == Millibronees('2000mbro')
...
def setup_gossip_store_test(node_factory, brocoind):
    ...
    wait_for(lambda: sum([c['base_fee_millibronees'] for c in l2.rpc.listchannels()['channels']]) == 21)
    ...
    wait_for(lambda: [c['base_fee_millibronees'] for c in l2.rpc.listchannels(scid23)['channels']] == [20, 20])
    ...
    wait_for(lambda: [c['base_fee_millibronees'] for c in l2.rpc.listchannels(scid12)['channels']] == [20, 20])
...
def test_gossip_store_upgrade_v7_v8(node_factory):
    ...
    assert l1.rpc.listchannels()['channels'] == [
        {'source': '022d223620a359a47ff7f7ac447c85c46c923da53389221a0054c11c1e3ca31d59',
         ...
         'public': False,
         'broneess': 1000000,
         'amount_mbro': Millibronees(1000000000),
         'message_flags': 1,
         'channel_flags': 0,
         'active': False,
         'last_update': 1588223664,
         'base_fee_millibronees': 1,
         'fee_per_millionth': 10,
         'delay': 6,
         'htlc_minimum_mbro': Millibronees(0),
         'htlc_maximum_mbro': Millibronees(990000000),
