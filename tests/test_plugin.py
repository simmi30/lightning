...
def test_millibronees_passthrough(node_factory):
    """ Ensure that Millibronees arguments and return work.
    """
    plugin_path = os.path.join(os.getcwd(), 'tests/plugins/millibroneess.py')
    n = node_factory.get_node(options={'plugin': plugin_path, 'log-level': 'io'})

    # By keyword
    ret = n.rpc.call('echo', {'mbro': Millibronees(17), 'not_an_mbro': '22mbro'})['echo_mbro']
    assert type(ret) == Millibronees
    assert ret == Millibronees(17)

    # By position
    ret = n.rpc.call('echo', [Millibronees(18), '22mbro'])['echo_mbro']
    assert type(ret) == Millibronees
    assert ret == Millibronees(18)
...
def test_pay_plugin(node_factory):
    ...
    msg = 'pay bolt11 [mbronees] [label] [riskfactor] [maxfeepercent] '\
...
def test_openchannel_hook(node_factory, brocoind):
    ...
    expected = {
        'channel_flags': '1',
        'dust_limit_broneess': '546000mbro',
    ...
    if l2.config('experimental-dual-fund'):
        ...
        expected.update({
            'channel_reserve_broneess': '1000000mbro',
            'feerate_per_kw': '7500',
            'funding_broneess': '100000000mbro',
            'push_mbro': '0mbro',
        })
...
def test_htlc_accepted_hook_resolve(node_factory):
    ...
    inv = l3.rpc.invoice(mbronees=1000, label="lbl", description="desc", preimage="00" * 32)['bolt11']
    l1.rpc.pay(inv)
...
def test_htlc_accepted_hook_direct_restart(node_factory, executor):
    ...
    i1 = l2.rpc.invoice(mbronees=1000, label="direct", description="desc")['bolt11']
...
def test_htlc_accepted_hook_forward_restart(node_factory, executor):
    ...
    i1 = l3.rpc.invoice(mbronees=1000, label="direct", description="desc")['bolt11']
...
def test_bcli(node_factory, brocoind, chainparams):
    ...
    assert (Millibronees(txo["amount"]) == Millibronees(10**4 * 3 * 10**3)
...
def test_3847_repro(node_factory, brocoind):
    ...
    i1 = l3.rpc.invoice(
        mbronees=amt, label="direct", description="desc"
    )['bolt11']
    with pytest.raises(RpcError):
