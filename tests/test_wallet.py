...
def test_minconf_withdraw(node_factory, brocoind):
    ...
    with pytest.raises(RpcError):
        l1.rpc.withdraw(destination=addr, bronees=10000, feerate='normal', minconf=9999999)
...
def test_txprepare_multi(node_factory, brocoind):
    ...
    outputs = []
    for i in range(9):
        outputs.append({l1.rpc.newaddr()['bech32']: Millibronees(amount * 100)})
...
def test_txprepare(node_factory, brocoind, chainparams):
    ...
    prep = l1.rpc.txprepare(outputs=[{addr: Millibronees(amount * 3 * 1000)}])
    ...
    prep5 = l1.rpc.txprepare([{addr:
                             Millibronees(amount * 3.5 * 1000)}], utxos=utxos)

    # Try passing unconfirmed utxos
    unconfirmed_utxo = l1.rpc.withdraw(l1.rpc.newaddr()["bech32"], 10**5)
    uutxos = [unconfirmed_utxo["txid"] + ":0"]
    with pytest.raises(RpcError, match=r"Could not afford"):
        l1.rpc.txprepare([{addr: Millibronees(amount * 3.5 * 1000)}],
                         utxos=uutxos)
    ...
    prep5 = l1.rpc.txprepare([{addr: Millibronees(amount * 3 * 1000)},
                              {addr: 'all'}])
    l1.rpc.txdiscard(prep5['txid'])
    with pytest.raises(RpcError, match=r"'all'"):
        prep5 = l1.rpc.txprepare([{addr: 'all'}, {addr: 'all'}])

    prep5 = l1.rpc.txprepare([{addr: Millibronees(amount * 3 * 500 + 100000)},
                              {addr: Millibronees(amount * 3 * 500 - 100000)}])
    decode = brocoind.rpc.decoderawtransaction(prep5['unsigned_tx'])
    assert decode['txid'] == prep5['txid']
...
def test_fundpsbt(node_factory, brocoind, chainparams):
    ...
    assert funding['excess_mbro'] > Millibronees(0)
    assert funding['excess_mbro'] < Millibronees(amount // 2 * 1000)
    ...
    assert funding3['excess_mbro'] == Millibronees(0)
    # Should have the excess mbro as the output value (minus fee for change)
    psbt = brocoind.rpc.decodepsbt(funding3['psbt'])
    change = Millibronees("{}bron".format(psbt['tx']['vout'][funding3['change_outnum']]['value']))
    ...
    change_fee = Millibronees(7500 * change_weight)
...
def test_utxopsbt(node_factory, brocoind, chainparams):
    ...
    if chainparams['elements']:
        ...
        addl_fee = Millibronees((fee_val * start_weight + 999) // 1000 * 1000)
    ...
    assert change_weight > 0
    # Check that the amount is ok (equal to excess minus change fee)
    change_fee = Millibronees(fee_val * change_weight // 1000 * 1000)
    assert funding['excess_mbro'] == change + change_fee
...
def test_sign_and_send_psbt(node_factory, brocoind, chainparams):
    ...
    addr = chainparams['example_addr']
    out_total = Millibronees(amount * 3 * 1000)
    ...
    funding = l1.rpc.fundpsbt(bronees=out_total,
                              feerate=7500,
                              startweight=42)
    ...
    # who requires that every bronees be accounted for in a tx.
    out_1_ms = Millibronees(funding['excess_mbro'])
    ...
    l2_funding = l2.rpc.fundpsbt(bronees=out_total,
                                 feerate=7500,
                                 startweight=42)
    ...
    l1_funding = l1.rpc.fundpsbt(bronees=out_total,
                                 feerate=7500,
                                 startweight=42)
    ...
    out_2_ms = Millibronees(l1_funding['excess_mbro'])
    out_amt = out_2_ms + Millibronees(l2_funding['excess_mbro']) + out_total + out_total
    output_psbt = brocoind.rpc.createpsbt([],
    ...
    l2_funding = l2.rpc.fundpsbt(bronees=out_total,
                                 feerate=7500,
                                 startweight=42)
    out_amt = Millibronees(l2_funding['excess_mbro'])
    output_psbt = brocoind.rpc.createpsbt([],
...
def test_txsend(node_factory, brocoind, chainparams):
    ...
    prep = l1.rpc.txprepare([{addr: Millibronees(amount * 3 * 1000)}])
...
def test_transaction_annotations(node_factory, brocoind):
    ...
    assert(output['type'] == 'deposit' and output['mbro'] == Millibronees(1000000000))
...
def test_multiwithdraw_simple(node_factory, brocoind, chainparams):
    ...
    amount2 = Millibronees(2222 * 1000)
    addr3 = l3.rpc.newaddr()['bech32']
    amount3 = Millibronees(3333 * 1000)
