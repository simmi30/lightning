...
def test_balance(node_factory):
    ...
    assert p1['mbronees_to_us'] == 10**6 * 1000
    assert p1['mbronees_total'] == 10**6 * 1000
    assert p2['mbronees_to_us'] == 0
    assert p2['mbronees_total'] == 10**6 * 1000
...
def test_reconnect_sender_add1(node_factory):
    ...
    route = [{'mbronees': amt, 'id': l2.info['id'], 'delay': 5, 'channel': '1x1x1'}]
...
def test_funding_close_upfront(node_factory, brocoind):
    ...
    def _fundchannel(l1, l2, amount, close_to):
        ...
        for node in [l1, l2]:
            ...
            assert amount * 1000 == channel['mbronees_total']
...
def test_multifunding_feerates(node_factory, brocoind):
    ...
    assert expected_fee == entry['fees']['base'] * 10 ** 8
    assert Millibronees(str(expected_fee) + 'bro') == txfee
...
def test_channel_persistence(node_factory, brocoind, executor):
    ...
    wait_for(lambda: only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronees_to_us'] == 99990000)
    ...
    # L1 doesn't actually update mbronees_to_us until it receives
    # revoke_and_ack from L2, which can take a little bit.
    wait_for(lambda: only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronees_to_us'] == 99980000)
    assert only_one(l2.rpc.listpeers()['peers'][0]['channels'])['mbronees_to_us'] == 20000

    # Finally restart l1, and make sure it remembers
    l1.restart()
    assert only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronees_to_us'] == 99980000
...
def test_feerate_stress(node_factory, executor):
    ...
    routel1l3 = [{'mbronees': '10002mbro', 'id': l2.info['id'], 'delay': 11, 'channel': scid12},
                 {'mbronees': '10000mbro', 'id': l3.info['id'], 'delay': 5, 'channel': scid23}]
    routel2l1 = [{'mbronees': '10000mbro', 'id': l1.info['id'], 'delay': 5, 'channel': scid12}]
...
def test_wumbo_channels(node_factory, brocoind):
    ...
    assert amount > Millibronees(str((1 << 24) - 1) + "bro")
...
def test_upgrade_statickey_fail(node_factory, executor, brocoind):
    ...
    # This HTLC will fail
    l1.rpc.sendpay([{'mbronees': 1000, 'id': l2.info['id'], 'delay': 5, 'channel': '1x1x1'}], '00' * 32, payment_secret='00' * 32)
...
def test_multichan(node_factory, executor, brocoind):
    ...
    assert chan23a['amount_mbro'] == Millibronees(1000000000)
    assert chan23a['short_channel_id'] == scid23a
    assert chan23b['amount_mbro'] == Millibronees(1001000000)
