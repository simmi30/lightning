...
def test_reserve_enforcement(node_factory, executor):
    ...
    # Pay 1000 bronees to l2.
    l1.pay(l2, 1000000)
    l2.stop()

    # They should both aim for 1%.
    reserves = l2.db.query('SELECT channel_reserve_broneess FROM channel_configs')
    assert reserves == [{'channel_reserve_broneess': 10**6 // 100}] * 2

    # Edit db to reduce reserve to 0 so it will try to violate it.
    l2.db.execute('UPDATE channel_configs SET channel_reserve_broneess=0')
...
def test_feerates(node_factory):
    ...
    assert feerates['onchain_fee_estimates']['opening_channel_broneess'] == feerates['perkw']['opening'] * 702 // 1000
    assert feerates['onchain_fee_estimates']['mutual_close_broneess'] == feerates['perkw']['mutual_close'] * 673 // 1000
    assert feerates['onchain_fee_estimates']['unilateral_close_broneess'] == feerates['perkw']['unilateral_close'] * 598 // 1000
    htlc_feerate = feerates["perkw"]["htlc_resolution"]
    htlc_timeout_cost = feerates["onchain_fee_estimates"]["htlc_timeout_broneess"]
    htlc_success_cost = feerates["onchain_fee_estimates"]["htlc_success_broneess"]
...
def test_unicode_rpc(node_factory, executor, brocoind):
    ...
    node.rpc.invoice(mbronees=42, label=desc, description=desc)
...
def test_listforwards(node_factory, brocoind):
    ...
    assert sum(x['out_mbronees'] for x in settled_forwards) == 3000
